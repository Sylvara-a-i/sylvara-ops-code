...
class ProductDirectionTests(unittest.TestCase):
    ...
    def test_product_direction_captures_the_accepted_boundary(self):
        required_markers = (
            ...
            "property management remain later candidates",
    ...
    def test_product_document_links_resolve_inside_the_repository(self):
        ...
        for source_path, source_text in (
            ...
            for raw_target in pattern.findall(source_text):
    ...
    def test_plumbing_first_direction_and_current_demo_gate_are_consistent(self):
        paths = (
        ...
        combined = "\n".join(texts)
        self.assertNotIn("Kansas City home-service operators", combined)
        self.assertNotIn("the first trade-specific template", self.product_text)
