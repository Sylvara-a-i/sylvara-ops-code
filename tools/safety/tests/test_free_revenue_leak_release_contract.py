...
class FreeRevenueLeakReleaseContractTests(unittest.TestCase):
    ...
    def test_forms_contract_matrix_is_exact_and_separates_live_observation(self):
        ...
        self.assertFalse(evidence["latest_sanitized_evidence_tenant_bound"])
        ...
        unbound_identity = identity["latest_unbound_observation"]
        self.assertEqual("Zoho Forms", unbound_identity["generation_owner"])
        self.assertIn("length 10", unbound_identity["generation_mechanism"])
        self.assertIn(
            "repetition restriction enabled",
            unbound_identity["generation_mechanism"],
        )
        self.assertFalse(unbound_identity["expected_tenant_binding_proven"])
    ...
    def test_non_retell_provider_preflights_are_sanitized_and_blocking(self):
        ...
        self.assertEqual(
            set(forms),
            {
                ...
                "latest_tenant_unbound_builder_observation_date",
                "latest_live_builder_evidence_remains_authoritative_within_its_recorded_limits",
                "tenant_unbound_observed_builder_state_refreshed",
        ...
        self.assertTrue(forms["tenant_unbound_observed_builder_state_refreshed"])
        self.assertFalse(forms["expected_tenant_live_state_refreshed"])
        self.assertFalse(forms["unobserved_live_state_inferred"])
        self.assertFalse(forms["expected_sylvara_forms_tenant_binding_proven"])
        self.assertEqual(forms["latest_live_builder_evidence_date"], "2026-08-26")
        self.assertEqual(
            forms["latest_tenant_unbound_builder_observation_date"],
            "2026-08-28",
        )
        ...
        self.assertEqual(
            set(forms_preflight),
            {
                ...
                "record_type",
                "observed_at",
                "repository_head_at_observation",
        ...
        self.assertEqual(
            forms_preflight["repository_head_at_observation"],
        ...
        self.assertEqual(
            ...
            "tenant_unbound_builder_observations_only",
        )
        ...
        self.assertIsNone(manifest_identity["live_generation_owner"])
        self.assertIsNone(manifest_identity["live_generation_mechanism"])
        manifest_unbound_identity = manifest_identity["latest_unbound_observation"]
        self.assertEqual(
            identity_preflight["generation_owner"],
            manifest_unbound_identity["generation_owner"],
        )
        self.assertEqual(
            manifest_unbound_identity["generation_mechanism"],
            "non-respondent provider-generated alphanumeric identifier; length 10; repetition restriction enabled",
        )
        self.assertFalse(manifest_unbound_identity["expected_tenant_binding_proven"])
