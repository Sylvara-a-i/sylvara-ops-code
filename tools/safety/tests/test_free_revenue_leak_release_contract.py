...
CONTRACT_MARKDOWN_PATH = (
    ROOT / "docs" / "product" / "free-revenue-leak-test-release-contract.md"
)
...
CRM_PIPELINE_BINDING_READBACK_PATH = (
    ROOT
    / "src"
    / "zoho-crm"
    / "free-revenue-leak-test"
    / "evidence"
    / "live-blueprint-pipeline-binding-readback-2026-08-28.json"
)
...
ANALYTICS_README_PATH = (
    ROOT / "src" / "zoho-catalyst" / "revenue-desk-analytics" / "README.md"
)
CURRENT_CONVERGENCE_PUBLIC_MIRROR_PATHS = (
    DEPLOYMENT_LOG_PATH,
    RECONCILIATION_RUNBOOK_PATH,
    SHARED_MONITOR_RUNBOOK_PATH,
    ZOHO_README_PATH,
...
FORM1_CRM_MAPPING_READBACK_PATH = (
    ROOT
    / "src"
    / "zoho-forms"
    / "free-revenue-leak-test"
    / "evidence"
    / "form1-crm-mapping-readback-2026-08-28.json"
)
FORM1_FIELD_POLICY_READBACK_PATH = (
    ROOT
    / "src"
    / "zoho-forms"
    / "free-revenue-leak-test"
    / "evidence"
    / "form1-field-policy-readback-2026-08-28.json"
)
...
FUNCTION_PACKAGE_SPECS = (
    ...
    (
        ...
        (
            ...
            ("revenue_desk_route_control", "Advanced I/O", "advancedio", "node24",
             "revenue_desk_route_control", "24.x"),
...
class FreeRevenueLeakReleaseContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.release_head_convergence = json.loads(
            RELEASE_HEAD_CONVERGENCE_PATH.read_text(encoding="utf-8")
        )
        ...
        cls.crm_pipeline_binding_readback = json.loads(
            CRM_PIPELINE_BINDING_READBACK_PATH.read_text(encoding="utf-8")
        )
        ...
        cls.current_convergence_public_mirrors = {
            path: path.read_text(encoding="utf-8")
            for path in CURRENT_CONVERGENCE_PUBLIC_MIRROR_PATHS
        }
        ...
        cls.form1_crm_mapping_readback = json.loads(
            FORM1_CRM_MAPPING_READBACK_PATH.read_text(encoding="utf-8")
        )
        cls.form1_field_policy_readback = json.loads(
            FORM1_FIELD_POLICY_READBACK_PATH.read_text(encoding="utf-8")
        )
    ...
    def test_forms_contract_matrix_is_exact_and_separates_live_observation(self):
        ...
        self.assertEqual(
            "historical authenticated read-only Zoho Forms preflight followed by separately approved bounded Form 2 containment, independent readback, and later no-save Form 1 CRM mapping and field-policy refreshes",
            evidence["live_readback_mode"],
        )
        self.assertEqual(
            "external read-only runtime observation with exploit-level details withheld",
            evidence["public_runtime_source"],
        )
        ...
        self.assertFalse(dedup["primary_generated_before_upsert_proven"])
        self.assertFalse(dedup["retry_duplicate_prevention_proven"])
        self.assertEqual(
            "upsert_order_observed_primary_generation_and_retry_safety_unproven",
            dedup["live_status"],
        )
        ...
        required_form1_crm_destinations = list(
            self.contract["form1"]["crm_field_mapping"].values()
        )
        self.assertEqual(len(required_form1_crm_destinations), 29)
        self.assertEqual(len(set(required_form1_crm_destinations)), 29)
        self.assertEqual(
            required_form1_crm_destinations,
            form1_fields["canonical_required_crm_destinations"],
        )
        self.assertEqual(form1_fields["live_crm_mapping_display_label_count_observed"], 30)
        self.assertTrue(form1_fields["live_crm_mapping_api_name_crosswalk_proven"])
        self.assertTrue(
            form1_fields["exact_missing_crm_api_destination_set_proven"]
        )
        self.assertEqual(form1_fields["exact_missing_crm_api_destination_set"], [])
        self.assertEqual(
            form1_fields["extra_noncanonical_mapping"],
            {
                "forms_field_label": "Additional Notes",
                "crm_api_name": "Free_Test_Request_Notes",
            },
        )
        self.assertEqual(
            [
        ...
        assisted = form1["assisted_prefill"]
        self.assertFalse(assisted["enabled"])
        self.assertTrue(assisted["source_candidate_enabled"])
        self.assertFalse(assisted["live_enabled"])
        ...
        self.assertTrue(
            assisted["submission_lanes"]["assisted"]
            ["consume_after_exact_crm_readback"]
        )
        self.assertTrue(all(value is False for value in assisted["live_readback"].values()))
        self.assertTrue(assisted["existing_controls_preserved_until_e2e_acceptance"])
        self.assertFalse(assisted["production_enabled"])
        release_assisted = self.contract["form1"]["assisted_path"]
        self.assertFalse(release_assisted["enabled"])
        self.assertTrue(release_assisted["source_candidate_enabled"])
        self.assertFalse(release_assisted["live_enabled"])
        self.assertEqual(release_assisted["routes"], assisted["routes"])
        self.assertEqual(
            release_assisted["controller_platform_data_dependencies"],
            "Catalyst Data Store plus distinct CRM read and write Connections",
        )
        self.assertEqual(
            self.contract["oauth_connection_references"][
        ...
        self.assertTrue(
            release_form1_fields["repository_source_transport_correction_complete"]
        )
        self.assertEqual(
            [
                "runtime read-only or respondent-editability semantics",
        ...
        self.assertEqual(
            "generation_mechanism_observed_retry_behavior_unproven",
            identity["status"],
        )
        self.assertEqual("Intake_Submission_ID", identity["field"])
        self.assertEqual("Zoho Forms", identity["live_generation_owner"])
        self.assertIn("length 10", identity["live_generation_mechanism"])
        self.assertIn(
            "repetition restriction enabled",
            identity["live_generation_mechanism"],
        )
        ...
        self.assertEqual(
            "builder_field_policy_dictionary_proven_runtime_retention_and_prefill_blocking",
            privacy["status"],
        )
        self.assertTrue(privacy["live_change_blocked"])
        self.assertEqual(privacy["live_readback"]["observed_deployed_field_count"], 26)
        self.assertEqual(privacy["live_readback"]["configured_alias_count"], 0)
        self.assertEqual(privacy["live_readback"]["disabled_or_locked_field_count"], 0)
        self.assertTrue(
            privacy["live_readback"][
                "complete_builder_type_alias_mandatory_personal_encryption_and_visibility_dictionary_proven"
            ]
        )
        self.assertTrue(
            privacy["live_readback"]["complete_builder_alias_dictionary_proven"]
        )
        self.assertFalse(privacy["live_readback"]["runtime_respondent_editability_proven"])
        self.assertFalse(privacy["live_readback"]["form_entry_retention_proven"])
        self.assertIn(
            "Intake_Submission_ID", privacy["minimum_named_field_scope"]
        )
        self.assertIn(
            "Assisted Intake Token", privacy["minimum_named_field_scope"]
        )
        self.assertIn(
            "personal classification", privacy["builder_properties_read_back_per_field"]
        )
        self.assertIn(
            "encryption setting", privacy["builder_properties_read_back_per_field"]
        )
        self.assertIn(
            "respondent editability", privacy["remaining_authority_gaps"]
        )
        ...
        self.assertFalse(form2["submission_webhook"]["live_contract_proven"])
        self.assertFalse(
            form2["submission_webhook"]["live_destination_binding_proven"]
        )
        self.assertFalse(form2["submission_webhook"]["live_authentication_proven"])
        self.assertFalse(
            form2["submission_webhook"]["live_prohibited_fields_absent_proven"]
        )
        ...
        self.assertEqual(
            "observed_noncompliant_private_details_withheld",
            matrix["live_read_only_flags"],
        )
        ...
        for key in ("prefillHandle", "prefillId", "configurationRevision"):
            ...
            self.assertEqual(
                {"required_policy_proven", "private_details_published"}, set(live)
            )
        ...
        for key in ("authorizedRepresentativeConfirmed", "testScopeAccepted"):
            source = respondent_policy[key]["source_policy"]
            self.assertTrue(source["mandatory"])
            self.assertTrue(source["personal"])
            self.assertFalse(source["encrypted"])
            self.assertTrue(source["respondent_editable"])
            self.assertFalse(source["default"])
            live = respondent_policy[key]["live_readback"]
            self.assertEqual(
                {"required_policy_proven", "private_details_published"}, set(live)
            )
            self.assertTrue(live["required_policy_proven"])
            self.assertFalse(live["private_details_published"])
            self.assertEqual(
                "observed_compliant_private_details_withheld",
                respondent_policy[key]["status"],
            )
        alert_policy = respondent_policy["alertRecipientEmail"]
        self.assertTrue(alert_policy["source_policy"]["mandatory"])
        self.assertTrue(alert_policy["source_policy"]["personal"])
        self.assertTrue(alert_policy["source_policy"]["encrypted"])
        self.assertTrue(alert_policy["source_policy"]["respondent_editable"])
        self.assertFalse(alert_policy["live_readback"]["required_policy_proven"])
        self.assertFalse(alert_policy["live_readback"]["private_details_published"])
        self.assertEqual("observed_noncompliant", alert_policy["status"])
        ...
        self.assertEqual(
            set(native),
            {
                ...
                "zoho_sign_allowed",
                "complete_required_native_action_policy_proven",
                "private_runtime_details_published",
                "status",
            },
        )
        for key in (
        ...
        self.assertEqual(native["status"], "blocked_private_revalidation_required")
    ...
    def test_exact_seven_function_and_two_pool_topology(self):
        expected_functions = [
        ...
        self.assertEqual(
            decisions["revenue_leak_test_request_form"],
            "canonical_active_development_production_dark",
        )
        self.assertEqual(
            decisions["revenue_desk_route_control"],
            "canonical_development_only",
        )
        self.assertTrue(
            all(
                decision == "canonical_active"
        ...
        form1_inventory = next(
            entry for entry in self.inventory["functions"]
            if entry["api_name"] == "revenue_leak_test_request_form"
        )
        self.assertEqual(
            form1_inventory["classification"],
            "canonical_active_development_production_dark_target",
        )
        self.assertEqual(
            form1_inventory["owner"], "assisted_and_public_form1_ingestion"
        )

        topology = self.inventory["topology_decision"]
        self.assertEqual(topology["canonical_project_count"], 1)
        self.assertEqual(topology["final_active_function_count"], 7)
        self.assertEqual(
    ...
    def test_executable_packages_match_the_seven_function_contract(self):
    ...
    def test_route_approval_and_activation_control_is_evidence_bound_and_out_of_band(self):
        ...
        self.assertEqual(inventory_control["function_count_increment"], 1)
        self.assertEqual(
            inventory_control["function"], "revenue_desk_route_control"
        )
        ...
        self.assertFalse(readiness["adds_public_gateway_route_or_worker_mode"])
    ...
    def test_canonical_tables_and_migration_boundary_are_exact(self):
        ...
        self.assertEqual(
            ...
            data["current_live_retained_unbound_form1_tables"],
        )
        self.assertEqual(
        ...
        self.assertEqual(
            [entry["runtime_variable"] for entry in request_schema["tables"]],
            ["SESSION_TABLE_NAME"],
        )
        self.assertEqual(
            [entry["runtime_binding"] for entry in request_schema["tables"]],
            ["exact-reviewed-table"],
        )
        self.assertEqual(
    ...
    def test_all_development_tables_have_lossless_fail_closed_dispositions(self):
        ...
        expected_newly_observed = {
            *contracts["canonical_call_tables"],
            *contracts["current_live_retained_unbound_form1_tables"],
        }
        self.assertEqual(
            schema_runtime_targets | schema_containment_targets,
            expected_newly_observed,
        )
    ...
    def test_six_function_development_deployment_is_exact_uninvoked_and_sanitized(self):
        ...
        self.assertEqual(
            ...
            "historical_six_function_evidence_preserved_seven_function_"
            "source_candidate_installation_pending",
        )
    ...
    def test_development_release_head_convergence_is_exact_uninvoked_and_sanitized(self):
        evidence = self.release_head_convergence
        revision = "288a93c7773acaf82fab277702e6b4e3d7354564"
        predecessor = "e1da1bc3457e506faf64c71db38869c8b26c1bbc"
        evidence_name = (
            "free-revenue-leak-test-development-pr-head-"
            "convergence-2026-08-28-288a93c.json"
        )
        ...
        self.assertEqual(evidence["execution_date_utc"], "2026-08-28")
        self.assertEqual(evidence["environment"], "Development")
        self.assertEqual(evidence["source_revision"], revision)
        self.assertEqual(evidence["deployed_pr_head_revision"], revision)
        self.assertEqual(evidence["predecessor_revision"], predecessor)
        self.assertEqual(
            evidence["outcome"],
            "six_function_release_head_source_and_configuration_convergence_"
            "exact_ingress_disabled_no_runtime_invocation",
        )
        self.assertEqual(
            evidence["release_classification"]["development_definition_convergence"],
            "proven_at_recorded_release_revision",
        )
        self.assertTrue(
            evidence["historical_evidence"]["records_preserved_without_rewrite"]
        )
        self.assertEqual(
            evidence["historical_evidence"]["immediate_predecessor_record"],
        ...
        deployment = evidence["deployment_readback"]
        expected_functions = [
            ("crm_billing_orchestrator", "Advanced I/O", "node24", 256, 30, 220),
            (
                "revenue_leak_test_request_form",
                "Advanced I/O",
                "node24",
            ...
            ("revenue_desk_call_gateway", "Advanced I/O", "node24", 256, 31, 27),
            ("revenue_desk_call_worker", "Job", "node24", 256, 28, 4),
            ("analytics_sync", "Job", "node24", 256, 7, 16),
        ]
        self.assertEqual(deployment["canonical_function_count"], 6)
        self.assertEqual(
            [
                (
                    ...
                    item["memory_mb"],
                    item["environment_variable_count"],
                    item["archive_file_count"],
                )
                for item in deployment["functions"]
            ],
            expected_functions,
        )
        self.assertEqual(deployment["configuration_delta"], "SOURCE_REVISION_only")
        self.assertTrue(
            deployment["all_non_revision_configuration_keys_and_values_preserved"]
        )
        self.assertTrue(
            all(
                item["configuration_source_revision_exact"]
                and item["archive_path_set_and_file_content_exact"]
                for item in deployment["functions"]
            )
        )
        functions_by_name = {
            item["api_name"]: item for item in deployment["functions"]
        }
        for name, item in functions_by_name.items():
            if name != "revenue_desk_call_worker":
                self.assertTrue(item["embedded_source_revision_exact"])
        self.assertEqual(
            functions_by_name["revenue_desk_call_worker"]
            ["embedded_source_revision_binding"],
            "shared_gateway_stamp_by_design",
        )
        for field in (
            "all_runtime_and_memory_readbacks_exact",
            "all_six_configuration_maps_exact",
            "all_six_archive_path_sets_exact",
            "all_six_archive_file_contents_exact",
            "final_fresh_six_function_readback_exact",
            "consumer_first_upload_order_proven",
        ):
            self.assertTrue(deployment[field])
        self.assertEqual(
            deployment["consumer_first_upload_order"],
            [item[0] for item in expected_functions],
        )
        self.assertFalse(deployment["final_main_parity_proven"])
        self.assertFalse(
            deployment["private_values_digests_identifiers_and_paths_included"]
        )

        authorization = evidence["authorization"]
        for field in (
            "approval_scope_finite",
            "approval_consumed_by_first_successful_crm_bundle_update",
            "approval_consumed",
            "approval_exhausted",
            "future_live_action_requires_fresh_scoped_approval",
        ):
        ...
        surfaces = evidence["execution_surfaces"]
        self.assertFalse(surfaces["connector_source_bundle_upload_or_archive_download_available"])
        self.assertTrue(surfaces["browser_fallback_used_only_for_missing_connector_capability"])
        self.assertFalse(surfaces["direct_rest_shell_or_browser_runtime_invocation_used"])
        ...
        safe = evidence["safe_configuration_readback"]
        self.assertEqual(safe["revenue_leak_test_setup_form"]["proof_mode"], "stub")
        self.assertFalse(safe["revenue_leak_test_setup_form"]["provider_send_invoked"])
        self.assertEqual(safe["revenue_desk_call_gateway"]["deployment_mode"], "active")
        self.assertFalse(safe["revenue_desk_call_gateway"]["api_gateway_ingress_enabled"])
        self.assertFalse(safe["revenue_desk_call_gateway"]["route_or_function_invoked"])
        self.assertEqual(
            safe["revenue_desk_call_worker"]["deployment_environment"],
            "development",
        )
        ...
        infrastructure = evidence["infrastructure_readback"]
        self.assertTrue(infrastructure["project_and_environment_binding_exact"])
        self.assertFalse(infrastructure["api_gateway_enabled"])
        self.assertTrue(
            infrastructure["api_gateway_connector_returned_positive_disabled_signal"]
        )
        ...
        self.assertFalse(infrastructure["current_route_count_claimed"])
        self.assertEqual(infrastructure["observed_cron_count"], 1)
        self.assertEqual(
            infrastructure["canonical_cron_name_pool_or_function_reference_count"],
            0,
        )
        self.assertEqual(infrastructure["observed_table_count"], 35)
        self.assertEqual(infrastructure["canonical_table_count"], 13)
        self.assertTrue(
            infrastructure["all_canonical_tables_present_exactly_once"]
        )
        self.assertFalse(infrastructure["table_schema_or_row_reconciliation_performed"])
        self.assertEqual(infrastructure["observed_job_pool_count"], 4)
        self.assertEqual(infrastructure["canonical_job_pool_count"], 2)
        self.assertTrue(
            infrastructure["all_canonical_job_pools_present_exactly_once"]
        )
        self.assertFalse(infrastructure["job_pool_function_target_binding_proven"])
        ...
        self.assertEqual(
            {item["surface"] for item in evidence["contained_ui_events"]},
            {
                "stale_function_row_action",
                "hidden_source_file_input",
                "code_editor_file_upload_path",
                "function_update_validation",
            },
        )
        self.assertFalse(
            next(
                item
                for item in evidence["contained_ui_events"]
                if item["surface"] == "stale_function_row_action"
            )["confirm_clicked"]
        )

        classification = evidence["release_classification"]
        self.assertEqual(classification["development_runtime_acceptance"], "not_proven")
        self.assertEqual(classification["final_main_parity"], "not_proven")
        self.assertEqual(classification["dark_production_readiness"], "not_proven")
        self.assertEqual(classification["retell_agent_testing_readiness"], "not_ready")
        self.assertEqual(classification["production_traffic_status"], "dark")
        self.assertEqual(
            evidence["disclosure_controls"],
            {
        ...
        for path, public_text in self.current_convergence_public_mirrors.items():
            with self.subTest(path=path):
                self.assertIn("288a93c", public_text)
                self.assertIn(evidence_name, public_text)
        self.assertEqual(self.pr_head_convergence["source_revision"], predecessor)
        self.assertIn(predecessor, self.deployment_log)
        self.assertIn(
            "free-revenue-leak-test-development-pr-head-convergence-2026-08-28.json",
            self.deployment_log,
        )
    ...
    def test_route_continuation_proves_exact_routes_and_preserves_dark_runtime(self):
        ...
        self.assertEqual(public_contract["schema_version"], 1)
        self.assertEqual(
        ...
        self.assertEqual(
            [item["id"] for item in route_contract["routes"]],
            canonical_route_ids,
        )
        profiles = route_contract["route_profiles"]
        self.assertEqual(set(profiles), {"canonical-all", "setup-journey"})
        ...
        self.assertFalse(setup_profile["retell_provider_activation_authorized"])
        self.assertFalse(setup_profile["production_gateway_activation_authorized"])
        ...
        self.assertEqual(
            inventory["public_route_contract_sha256"],
            ROUTE_CONTRACT_SHA256_AT_SOURCE_REVISION,
        )
        self.assertEqual(
        ...
        exact_public_mirror_clauses = {
            ...
            CALL_RUNTIME_README_PATH: (
                ...
                "a relative prior-24-hour access and application log query ran after the final disabled-gateway readback",
                "exact utc bounds were not retained",
                "during these executions, the operator invoked no route, function, job, or cron",
            ),
        }
    ...
    def test_non_retell_provider_preflights_are_sanitized_and_blocking(self):
        ...
        self.assertFalse(boundary["metered_billing_state_connector_field_available"])
        self.assertTrue(boundary["metered_billing_state_verified_privately_in_settings_ui"])
        self.assertFalse(boundary["metered_billing_enabled"])
        self.assertFalse(
            boundary["metered_billing_observation_is_usage_billing_evidence"]
        )
        self.assertFalse(boundary["usage_billing_state_verified"])
        self.assertTrue(
            boundary["installed_sylvara_billing_changes_connector_reviewed"]
        )
        for field in (
            "installed_product_create_tool_available",
            "installed_plan_create_tool_available",
            "installed_addon_create_tool_available",
            "installed_usage_billing_settings_tool_available",
            "installed_mark_inactive_tool_available",
        ):
            self.assertFalse(boundary[field])
        self.assertTrue(boundary["browser_fallback_used_for_metered_billing_state_only"])
        self.assertTrue(boundary["customer_subscription_invoice_and_addon_catalogs_read"])
        self.assertEqual(
            (
                boundary["customer_count"],
                boundary["subscription_count"],
                boundary["invoice_count"],
            ),
            (0, 0, 0),
        )
        ...
        self.assertEqual(target["connected_minute_usage_addon_type"], "recurring")
        self.assertEqual(target["connected_minute_usage_addon_pricing_scheme"], "unit")
        self.assertEqual(target["connected_minute_usage_addon_interval"], 1)
        self.assertEqual(target["connected_minute_usage_addon_interval_unit"], "months")
        self.assertTrue(
            target["connected_minute_usage_addon_readback_requires_is_usage_supported"]
        )
        self.assertTrue(target["usage_billing_enabled_precondition_for_usage_addon"])
        self.assertFalse(target["usage_billing_enabled_precondition_satisfied"])
        self.assertFalse(
            target[
                "typed_catalog_write_readback_and_mark_inactive_capabilities_proven"
            ]
        )
        self.assertFalse(target["reviewed_per_operation_contract_registry_configured"])
        self.assertEqual(
            target["committed_capability_authority_status"],
            "blocked_no_reviewed_installed_contract",
        )
        self.assertFalse(target["committed_capability_authority_executable"])
        self.assertFalse(target["caller_substituted_registry_can_authorize_mutation"])
        self.assertFalse(target["packet_supplied_contract_hashes_are_authority"])
        self.assertTrue(target["schema_v4_packet_authorizes_exactly_one_target"])
        self.assertTrue(target["fresh_complete_product_absence_prestate_required"])
        self.assertTrue(target["fresh_missing_tier_prestate_required_for_each_plan"])
        self.assertTrue(
            target["present_plan_inventory_requires_concrete_exact_ids_and_terms"]
        )
        self.assertTrue(target["usage_addon_requires_all_three_concrete_exact_plans"])
        self.assertTrue(
            target["fresh_complete_usage_addon_absence_prestate_required"]
        )
        self.assertFalse(target["plan_retry_or_bulk_operation_allowed"])
        self.assertTrue(
            target["resolved_ambiguous_absence_requires_prior_packet_binding"]
        )
        self.assertFalse(target["resolved_existing_target_allows_creation"])
        self.assertFalse(target["durable_approval_consumption_executor_proven"])
        self.assertFalse(
            target["durable_consumption_identity_is_approval_timestamp_dependent"]
        )
        self.assertTrue(target["stable_authority_id_returned_for_unique_ledger_key"])
        self.assertTrue(
            target["consumption_digest_binds_stable_authority_id_and_exact_packet"]
        )
        self.assertFalse(
            target["separate_untracked_packet_digest_required_for_consumption"]
        )
        self.assertTrue(target["mutation_validator_currently_fails_closed"])
        for field in (
            ...
            "declarative_single_use_is_execution_consumption_proof",
            "catalog_validator_success_without_durable_executor_claim_allows_mutation",
        ):
            self.assertFalse(billing["containment"][field])
        self.assertTrue(any(
            "one schema-v4 plans-phase packet per still-missing exact tier" in packet
            for packet in billing["required_next_packets"]
        ))
        self.assertTrue(any(
            "durable private operation-authorization-consumption executor" in packet
            for packet in billing["required_next_packets"]
        ))
        self.assertFalse(boundary["payment_event_or_report_record_content_read"])
        ...
        self.assertEqual(
            set(forms),
            {
                ...
                "latest_live_builder_evidence_date",
                "latest_live_builder_evidence_remains_authoritative_within_its_recorded_limits",
                "tenant_bound_observed_builder_state_refreshed",
                "expected_tenant_live_state_refreshed",
                "unobserved_live_state_inferred",
                "expected_sylvara_forms_tenant_binding_proven",
                "known_form1_blockers_remain",
                "known_form2_blockers_remain",
        ...
        self.assertTrue(forms["browser_fallback_used"])
        self.assertEqual(
            forms["browser_fallback_reason"],
            "No purpose-built Sylvara Zoho Forms connector was advertised; Zoho Creator form metadata is a different product and was not substituted.",
        )
        self.assertTrue(
            forms[
                "latest_live_builder_evidence_remains_authoritative_within_its_recorded_limits"
        ...
        self.assertTrue(forms["tenant_bound_observed_builder_state_refreshed"])
        self.assertTrue(forms["expected_tenant_live_state_refreshed"])
        self.assertFalse(forms["unobserved_live_state_inferred"])
        self.assertTrue(forms["expected_sylvara_forms_tenant_binding_proven"])
        self.assertEqual(forms["latest_live_builder_evidence_date"], "2026-08-28")
        self.assertTrue(forms["known_form1_blockers_remain"])
        self.assertTrue(forms["known_form2_blockers_remain"])

        forms_preflight = self.forms_browser_preflight
        self.assertEqual(
            set(forms_preflight),
            {
                "schema_version",
                "record_type",
                "observed_at",
                "repository_head_at_builder_observation",
                "repository_head_at_tenant_binding_readback",
                "release_contract_at_observation",
                "status",
                "tenant_binding",
                "evidence_boundary",
                "form1",
        ...
        self.assertEqual(
            set(forms_preflight["release_contract_at_observation"]),
            {"contract_id", "schema_version"},
        )
        self.assertEqual(forms_preflight["schema_version"], 2)
        self.assertEqual(
            forms_preflight["record_type"],
            "sanitized_free_revenue_leak_test_zoho_forms_read_only_browser_fallback_preflight",
        )
        self.assertEqual(forms_preflight["observed_at"], "2026-08-28")
        self.assertEqual(
            forms_preflight["repository_head_at_builder_observation"],
            "caef1d81567cbc2d8ef02e479a299fe849d2ccf5",
        )
        self.assertEqual(
            forms_preflight["repository_head_at_tenant_binding_readback"],
            "caef1d81567cbc2d8ef02e479a299fe849d2ccf5",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            forms_preflight["status"],
            "expected_tenant_bound_current_builder_and_public_runtime_blockers_confirmed",
        )
        tenant_binding = forms_preflight["tenant_binding"]
        self.assertEqual(
            set(tenant_binding),
            {
                "verified_privately",
                "authenticated_business_account_match",
                "tenant_namespace_match",
                "canonical_form_title_match",
                "private_account_tenant_namespace_or_user_value_included",
            },
        )
        self.assertTrue(tenant_binding["verified_privately"])
        self.assertTrue(tenant_binding["authenticated_business_account_match"])
        self.assertTrue(tenant_binding["tenant_namespace_match"])
        self.assertTrue(tenant_binding["canonical_form_title_match"])
        self.assertFalse(
            tenant_binding["private_account_tenant_namespace_or_user_value_included"]
        )
        forms_boundary = forms_preflight["evidence_boundary"]
        self.assertEqual(
            set(forms_boundary),
            {
                "sylvara_forms_audit_connector_available",
                "sylvara_forms_changes_connector_available",
                "zoho_creator_form_metadata_substituted",
                "fallback_reason",
                "source",
                "public_runtime_source",
                "catalyst_binding_comparison_source",
                "catalyst_binding_compared_without_runtime_invocation",
                "authenticated_account_identity_observed_privately",
                "expected_sylvara_forms_tenant_binding_proven",
                "private_account_tenant_or_user_identity_included",
                "canonical_active_form_count",
                "bounded_observed_builder_state_refreshed",
                "public_runtime_read_only_render_checked",
                "unobserved_builder_or_tenant_state_inferred",
                "private_form_ids_urls_aliases_destinations_headers_or_values_included",
                "customer_or_submission_records_read",
                "public_runtime_fields_or_values_entered",
                "public_runtime_save_next_or_submit_clicked",
                "writes_saves_submissions_webhook_tests_or_runtime_invocations_performed",
                "retell_customer_billing_or_production_action_performed",
            },
        )
        self.assertFalse(forms_boundary["sylvara_forms_audit_connector_available"])
        self.assertFalse(forms_boundary["sylvara_forms_changes_connector_available"])
        self.assertFalse(forms_boundary["zoho_creator_form_metadata_substituted"])
        self.assertEqual(
            forms_boundary["fallback_reason"],
            "No purpose-built Sylvara Zoho Forms connector was advertised for the required builder and integration metadata.",
        )
        self.assertEqual(
            forms_boundary["source"],
        ...
        self.assertEqual(
            forms_boundary["public_runtime_source"],
            "external read-only runtime observation with exploit-level details withheld",
        )
        self.assertEqual(
            forms_boundary["catalyst_binding_comparison_source"],
            "authenticated read-only Catalyst project, function, route, and environment binding metadata",
        )
        self.assertTrue(
            forms_boundary["catalyst_binding_compared_without_runtime_invocation"]
        )
        self.assertTrue(
            forms_boundary["authenticated_account_identity_observed_privately"]
        )
        self.assertTrue(
            forms_boundary["expected_sylvara_forms_tenant_binding_proven"]
        )
        self.assertFalse(
            forms_boundary["private_account_tenant_or_user_identity_included"]
        )
        self.assertEqual(forms_boundary["canonical_active_form_count"], 2)
        self.assertTrue(forms_boundary["bounded_observed_builder_state_refreshed"])
        self.assertTrue(forms_boundary["public_runtime_read_only_render_checked"])
        self.assertFalse(forms_boundary["unobserved_builder_or_tenant_state_inferred"])
        self.assertFalse(
            forms_boundary[
                "private_form_ids_urls_aliases_destinations_headers_or_values_included"
            ]
        )
        self.assertFalse(forms_boundary["customer_or_submission_records_read"])
        self.assertFalse(forms_boundary["public_runtime_fields_or_values_entered"])
        self.assertFalse(forms_boundary["public_runtime_save_next_or_submit_clicked"])
        self.assertFalse(
            forms_boundary[
                "writes_saves_submissions_webhook_tests_or_runtime_invocations_performed"
            ]
        ...
        self.assertFalse(
            forms_boundary["retell_customer_billing_or_production_action_performed"]
        )
        form1_preflight = forms_preflight["form1"]
        self.assertEqual(
            set(form1_preflight),
            {
                "public_submission_identity",
                "field_alias_configuration",
                "field_inventory",
                "assisted_token",
                "native_crm_integration",
                "complete_privacy_and_read_only_dictionary_proven",
            },
        )
        identity_preflight = form1_preflight["public_submission_identity"]
        self.assertEqual(
            set(identity_preflight),
            {
                "field_present",
                "respondent_visible",
                "generation_owner",
                ...
                "configured_length",
                "restrict_id_repetition",
                "mechanism_observed",
                "generated_before_native_crm_upsert_proven",
                "stable_across_provider_retry_or_repush_proven",
                "duplicate_lead_prevention_proven",
            },
        )
        self.assertTrue(identity_preflight["field_present"])
        self.assertFalse(identity_preflight["respondent_visible"])
        self.assertEqual(identity_preflight["generation_owner"], "Zoho Forms")
        self.assertEqual(
            identity_preflight["generation_mechanism"],
            "provider-generated alphanumeric identifier",
        )
        self.assertTrue(identity_preflight["mechanism_observed"])
        self.assertEqual(identity_preflight["configured_length"], 10)
        self.assertTrue(identity_preflight["restrict_id_repetition"])
        self.assertFalse(identity_preflight["generated_before_native_crm_upsert_proven"])
        self.assertFalse(identity_preflight["stable_across_provider_retry_or_repush_proven"])
        self.assertFalse(identity_preflight["duplicate_lead_prevention_proven"])
        self.assertFalse(form1_preflight["complete_privacy_and_read_only_dictionary_proven"])
        self.assertEqual(
            set(form1_preflight["field_alias_configuration"]),
            {"configured_alias_count", "assisted_token_alias_present"},
        )
        self.assertEqual(
            form1_preflight["field_alias_configuration"]["configured_alias_count"],
            0,
        )
        self.assertFalse(
            form1_preflight["field_alias_configuration"]["assisted_token_alias_present"]
        )
        self.assertTrue(form1_preflight["assisted_token"]["field_present"])
        self.assertEqual(
            set(form1_preflight["assisted_token"]),
            {
                "field_present",
                "mandatory",
                "personal",
                "encrypted",
                "prefill_webhook_field_present",
            },
        )
        self.assertFalse(form1_preflight["assisted_token"]["mandatory"])
        self.assertFalse(form1_preflight["assisted_token"]["personal"])
        self.assertFalse(form1_preflight["assisted_token"]["encrypted"])
        self.assertFalse(
            form1_preflight["assisted_token"]["prefill_webhook_field_present"]
        )
        form1_inventory = form1_preflight["field_inventory"]
        self.assertEqual(
            set(form1_inventory),
            {
                "observed_deployed_field_count",
                "mandatory_true_count",
                "personal_true_count",
                "encrypted_true_count",
                "default_show_count",
                "default_hide_count",
                "provider_generated_or_no_visibility_control_count",
                "missing_optional_attribution_fields",
                "all_observed_properties_discarded_without_save",
            },
        )
        self.assertEqual(form1_inventory["observed_deployed_field_count"], 26)
        self.assertEqual(form1_inventory["mandatory_true_count"], 10)
        self.assertEqual(form1_inventory["personal_true_count"], 7)
        self.assertEqual(form1_inventory["encrypted_true_count"], 0)
        self.assertEqual(form1_inventory["default_show_count"], 14)
        self.assertEqual(form1_inventory["default_hide_count"], 11)
        self.assertEqual(
            form1_inventory["provider_generated_or_no_visibility_control_count"], 1
        )
        self.assertEqual(
            form1_inventory["missing_optional_attribution_fields"],
            ["utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term"],
        )
        self.assertTrue(form1_inventory["all_observed_properties_discarded_without_save"])

        form1_crm = form1_preflight["native_crm_integration"]
        self.assertEqual(
            set(form1_crm),
            {
                "configured",
                "expected_organization_matched_privately",
                "module",
                "layout",
                "observed_mapping_count",
                "observed_mappings",
                "observed_mappings_are_display_labels",
                "observed_mapping_api_name_crosswalk_proven",
                "exact_missing_api_name_count_proven",
                "required_full_mapping_present",
                "upsert_enabled",
                "upsert_preference_order",
                "blank_overwrite",
                "crm_automation_and_process_management_enabled",
                "attachments_signatures_approval_gclid_assignment_and_tags_enabled",
                "integration_write_or_test_performed",
            },
        )
        self.assertTrue(form1_crm["configured"])
        self.assertTrue(form1_crm["expected_organization_matched_privately"])
        self.assertEqual(form1_crm["module"], "Leads")
        self.assertEqual(form1_crm["layout"], "Standard")
        self.assertEqual(form1_crm["observed_mapping_count"], 6)
        self.assertEqual(
            form1_crm["observed_mappings"],
            [
                {"crm_field": "Lead Status", "form_field": "Lead Status"},
                {"crm_field": "Lead Source", "form_field": "Lead Source"},
                {"crm_field": "Company", "form_field": "Company"},
                {"crm_field": "Contact Email", "form_field": "Email"},
                {"crm_field": "First Name", "form_field": "Name - First Name"},
                {"crm_field": "Last Name", "form_field": "Name - Last Name"},
            ],
        )
        self.assertTrue(form1_crm["observed_mappings_are_display_labels"])
        self.assertFalse(form1_crm["observed_mapping_api_name_crosswalk_proven"])
        self.assertFalse(form1_crm["exact_missing_api_name_count_proven"])
        self.assertFalse(form1_crm["required_full_mapping_present"])
        self.assertTrue(form1_crm["upsert_enabled"])
        self.assertEqual(
            form1_crm["upsert_preference_order"],
            ["Intake Submission ID", "Contact Email"],
        )
        self.assertFalse(form1_crm["blank_overwrite"])
        self.assertTrue(form1_crm["crm_automation_and_process_management_enabled"])
        self.assertFalse(
            form1_crm["attachments_signatures_approval_gclid_assignment_and_tags_enabled"]
        )
        self.assertFalse(form1_crm["integration_write_or_test_performed"])
        form2_preflight = forms_preflight["form2"]
        self.assertEqual(
            set(form2_preflight),
            {
                "status",
                "complete_required_field_policy_proven",
                "required_prefill_boundary_proven",
                "required_controller_field_policy_proven",
                "required_respondent_field_policy_proven",
                "required_controller_gate_proven",
                "required_submission_webhook_contract_proven",
                "required_submission_destination_binding_proven",
                "required_submission_authentication_proven",
                "read_only_public_runtime_observation_performed",
                "private_exploit_details_included",
                "field_value_entered",
                "save_next_or_submit_clicked",
                "webhook_test_performed",
            },
        )
        self.assertEqual(
        ...
        for field in (
            ...
            self.assertFalse(form2_preflight[field], field)
        self.assertTrue(form2_preflight["read_only_public_runtime_observation_performed"])
        manifest_forms = {
            entry["logical_name"]: entry for entry in self.forms_manifest["forms"]
        }
        manifest_form1 = manifest_forms["REVENUE_LEAK_TEST_REQUEST_FORM"]
        manifest_identity = manifest_form1[
            "live_configuration_prerequisites"
        ]["public_intake_submission_identity"]
        self.assertEqual(
            identity_preflight["generation_owner"],
            manifest_identity["live_generation_owner"],
        )
        self.assertEqual(
            manifest_identity["live_generation_mechanism"],
            "non-respondent provider-generated alphanumeric identifier; length 10; repetition restriction enabled",
        )
        manifest_form1_crm = manifest_form1["crm_integration"]["live_readback"]
        for field in (
            "configured",
            "expected_organization_matched_privately",
            "module",
            ...
            self.assertEqual(form1_crm[field], manifest_form1_crm[field], field)
        current_mapping = self.form1_crm_mapping_readback
        self.assertEqual(
            self.forms_manifest["evidence_boundary"]["form1_crm_mapping_readback"],
            "evidence/form1-crm-mapping-readback-2026-08-28.json",
        )
        self.assertEqual(
            current_mapping["status"],
            "crm_mapping_and_upsert_contract_satisfied_remaining_form1_gates_blocking",
        )
        self.assertEqual(manifest_form1_crm["observed_mapping_count"], 30)
        self.assertEqual(len(manifest_form1_crm["observed_mappings"]), 30)
        self.assertTrue(manifest_form1_crm["observed_mapping_api_name_crosswalk_proven"])
        self.assertTrue(manifest_form1_crm["exact_missing_api_name_count_proven"])
        self.assertTrue(manifest_form1_crm["required_full_mapping_present"])
        self.assertEqual(manifest_form1_crm["missing_required_api_names"], [])
        self.assertEqual(
            manifest_form1_crm["extra_mapping_api_name"], "Free_Test_Request_Notes"
        )
        current_mapping_contract = current_mapping["mapping_readback"]
        self.assertEqual(current_mapping_contract["observed_mapping_count"], 30)
        self.assertEqual(current_mapping_contract["canonical_required_destination_count"], 29)
        self.assertEqual(
            current_mapping_contract["canonical_crm_api_names"],
            manifest_form1["field_contract"]["canonical_required_crm_destinations"],
        )
        self.assertTrue(
            current_mapping_contract[
                "all_canonical_required_destinations_mapped_exactly_once"
            ]
        )
        self.assertEqual(current_mapping_contract["missing_canonical_destinations"], [])
        self.assertFalse(
            current_mapping["authority_limit"]["future_live_change_authorized_by_this_record"]
        )
        current_field_policy = self.form1_field_policy_readback
        self.assertEqual(
            self.forms_manifest["evidence_boundary"]["form1_field_policy_readback"],
            "evidence/form1-field-policy-readback-2026-08-28.json",
        )
        self.assertEqual(
            current_field_policy["status"],
            "builder_field_policy_dictionary_proven_runtime_retention_and_prefill_blocking",
        )
        self.assertEqual(len(current_field_policy["fields"]), 26)
        self.assertTrue(
            all(field["alias_configured"] is False for field in current_field_policy["fields"])
        )
        self.assertFalse(current_field_policy["prefill_readback"]["prefill_webhook_field_present"])
        self.assertFalse(
            current_field_policy["prefill_readback"][
                "existing_assisted_intake_token_field_safe_to_repurpose"
            ]
        )
        self.assertTrue(
            current_field_policy["authority_limit"][
                "complete_builder_type_alias_mandatory_personal_encryption_and_visibility_dictionary_proven"
            ]
        )
        self.assertFalse(
            current_field_policy["authority_limit"]["runtime_respondent_editability_proven"]
        )
        self.assertFalse(
            current_field_policy["authority_limit"]["future_live_change_authorized_by_this_record"]
        )
        manifest_form1_inventory = manifest_form1["live_configuration_prerequisites"][
            "privacy_dictionary"
        ]["live_readback"]
        for field in (
            "observed_deployed_field_count",
            ...
            self.assertEqual(form1_inventory[field], manifest_form1_inventory[field], field)
        self.assertEqual(
            form1_preflight["field_alias_configuration"]["configured_alias_count"],
        ...
        self.assertEqual(
            manifest_form2["live_field_inventory"],
            {
                ...
                "complete_private_runtime_dictionary_retained_outside_github": False,
                "private_exploit_details_published": False,
                "status": "blocked_private_revalidation_required",
            },
        )
        self.assertEqual(
            manifest_form2["live_field_rules"],
            {
                "required_controller_gate_rule_proven": False,
                "private_rule_inventory_published": False,
                "status": "blocked_private_revalidation_required",
            },
        )
        self.assertEqual(
            manifest_form2["live_runtime_security_boundary"],
            {
                "read_only_observation_performed": True,
                "required_controller_gate_proven": False,
                "required_field_exposure_policy_proven": False,
                "private_exploit_details_published": False,
        ...
        for entry in manifest_form2["controller_field_policy"]:
            self.assertEqual(
                entry["live_readback"],
                {"required_policy_proven": False, "private_details_published": False},
            )
        respondent_live_policy = {
            entry["key"]: entry["live_readback"]
            for entry in manifest_form2["respondent_field_policy"]
        }
        self.assertEqual(
            respondent_live_policy,
            {
                "authorizedRepresentativeConfirmed": {
                    "required_policy_proven": True,
                    "private_details_published": False,
                },
                "testScopeAccepted": {
                    "required_policy_proven": True,
                    "private_details_published": False,
                ...
                "alertRecipientEmail": {
                    "required_policy_proven": False,
                    "private_details_published": False,
                },
            },
        )
        manifest_webhook = manifest_form2["submission_webhook"]
        self.assertFalse(manifest_webhook["live_contract_proven"])
        self.assertFalse(manifest_webhook["live_destination_binding_proven"])
        self.assertFalse(manifest_webhook["live_authentication_proven"])
        self.assertFalse(manifest_webhook["live_prohibited_fields_absent_proven"])
        self.assertFalse(manifest_webhook["live_private_details_published"])
        self.assertFalse(manifest_webhook["webhook_test_performed"])
        self.assertEqual(
            set(forms_preflight["containment"]),
            {
                "builder_change_authorized_by_this_record",
                "webhook_change_authorized_by_this_record",
                "gateway_activation_authorized_by_this_record",
                "synthetic_submission_authorized_by_this_record",
                "future_live_change_authorized_by_this_record",
            },
        )
        for authorized in forms_preflight["containment"].values():
        ...
        for phrase in (
            "No purpose-built Sylvara Forms connector was available",
            "one unrelated product, one unrelated active monthly plan, and zero customers, subscriptions, invoices, or add-ons",
            "historical UI readback covered legacy Metered Billing only and is not Usage Billing evidence",
            "fresh complete product/add-on absence",
            "registry must match the committed capability-authority digest",
            "installed connector's missing product, plan, add-on, Usage Billing settings, and mark-inactive tools",
            "no durable executor currently claims the stable operation-authorization ID",
        ...
        def collect_json_keys(value):
            if isinstance(value, dict):
                for key, nested in value.items():
                    yield str(key).lower()
                    yield from collect_json_keys(nested)
            elif isinstance(value, list):
                for nested in value:
                    yield from collect_json_keys(nested)
        ...
        for forbidden_key in (
            ...
            "account_id", "tenant_id", "user_id", "form_id", "provider_id",
            "destination", "header", "alias", "email", "phone", "url",
            "client_secret", "refresh_token", "access_token", "zcfkey",
        ):
        ...
        self.assertIsNone(
            re.search(r"[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}", combined)
        )
        self.assertIsNone(
            re.search(
    ...
    def test_successor_crm_and_catalyst_preflights_close_only_proven_gates(self):
        ...
        current_pipeline = self.crm_pipeline_binding_readback
        self.assertEqual(
            current_pipeline["status"],
            "pipeline_binding_satisfied_blueprint_topology_and_runtime_blocking",
        )
        self.assertTrue(
            current_pipeline["pipeline_binding_readback"][
                "pipeline_binding_matches_revenue_desk_sales"
            ]
        )
        self.assertFalse(current_pipeline["authority_limit"]["topology_matches_contract"])
        self.assertEqual(
            current_pipeline["authority_limit"]["missing_expected_transitions"],
            ["Record Internal Approval", "Activate Test Route"],
        )
        self.assertFalse(
            current_pipeline["authority_limit"]["future_live_change_authorized_by_this_record"]
        )
    ...
    def test_form1_source_candidate_and_live_table_disposition_stay_distinct(self):
        registry = json.loads((
            ROOT / "src" / "zoho-catalyst" / "revenue-leak-test-request-form"
        ...
        authentication_variables = {
            ...
            "FORM1_PREFILL": "PREFILL_HEADER_SECRET",
            "FORM1_SUBMISSION": "SUBMISSION_HEADER_SECRET",
        }
        ...
        self.assertIn("MAX_BODY_BYTES", variable_names)
        self.assertIn("SESSION_TABLE_NAME", variable_names)
        self.assertIn("CRM_READ_CONNECTION_LINK_NAME", variable_names)
        self.assertIn("CRM_WRITE_CONNECTION_LINK_NAME", variable_names)

        form1_schema = json.loads((
            ROOT / "src" / "zoho-catalyst" / "revenue-leak-test-request-form"
            / "config" / "datastore-schema.json"
        ).read_text(encoding="utf-8"))
        self.assertEqual(
        ...
        self.assertEqual(
            self.contract["catalyst_data_contracts"]
            ["current_live_retained_unbound_form1_tables"],
            [table_name],
        )
        self.assertEqual(
            self.inventory["canonical_tables"]["form1_active_source_candidate"],
    ...
    def test_billing_mapping_and_acceptance_gate_remain_exact(self):
        ...
        self.assertEqual(billing["private_catalog_packet_schema_version"], 4)
        self.assertEqual(
            billing["private_catalog_packet_phases"],
            ["product", "plans", "usage_addon"],
        )
        self.assertEqual(billing["private_catalog_packet_sequence"], [
            "one_product_packet_if_missing",
            "one_plan_packet_per_still_missing_tier",
            "one_usage_addon_packet_after_all_plans_read_back_exact",
        ])
        product_packet = billing["product_packet_contract"]
        self.assertEqual(product_packet["maximum_packet_count"], 1)
        self.assertEqual(
            product_packet["authorized_operation"], "create_test_product"
        )
        self.assertTrue(
            product_packet["fresh_complete_paginated_product_prestate_required"]
        )
        self.assertTrue(product_packet["zero_exact_target_name_matches_required"])
        plan_packet = billing["plan_packet_contract"]
        self.assertEqual(plan_packet["authorized_operation"], "create_test_plan")
        self.assertEqual(plan_packet["target_plan_tier_count"], 1)
        self.assertTrue(plan_packet["retains_all_three_commercial_definitions"])
        self.assertTrue(plan_packet["fresh_complete_paginated_prestate_required"])
        self.assertTrue(plan_packet["canonical_concrete_plan_inventory_required"])
        self.assertTrue(
            plan_packet["present_tier_requires_unique_plan_id_and_exact_normalized_terms"]
        )
        self.assertTrue(
            plan_packet["zero_exact_target_code_and_name_matches_required"]
        )
        self.assertFalse(plan_packet["retry_allowed"])
        self.assertTrue(
            plan_packet[
                "resolved_ambiguity_requires_prior_packet_digest_and_authoritative_evidence"
            ]
        )
        self.assertFalse(plan_packet["resolved_existing_target_allows_creation"])
        self.assertTrue(
            plan_packet[
                "partial_success_requires_fresh_packet_for_different_still_missing_tier"
            ]
        )
        addon_packet = billing["usage_addon_packet_contract"]
        self.assertEqual(addon_packet["maximum_packet_count"], 1)
        self.assertEqual(
            addon_packet["authorized_operation"], "create_test_usage_addon"
        )
        self.assertTrue(
            addon_packet["all_three_concrete_plans_must_read_back_exact_first"]
        )
        self.assertTrue(
            addon_packet["fresh_complete_paginated_addon_prestate_required"]
        )
        self.assertTrue(
            addon_packet["zero_exact_target_code_and_name_matches_required"]
        )
        self.assertTrue(billing["usage_billing_enabled_precondition_for_usage_addon"])
        self.assertFalse(billing["legacy_metered_billing_is_usage_billing_evidence"])
        self.assertIn(
            "feature=usage_billing", billing["usage_billing_precondition_evidence"]
        )
        attestation = billing["usage_billing_attestation_contract"]
        self.assertEqual(attestation["schema_version"], 1)
        self.assertEqual(attestation["environment"], "TEST")
        self.assertEqual(attestation["source"], "authenticated_billing_settings_ui")
        self.assertEqual(attestation["feature"], "usage_billing")
        self.assertTrue(attestation["usage_billing_enabled"])
        self.assertTrue(attestation["usage_tracking_addon_control_available"])
        self.assertTrue(attestation["enterprise_entitlement_proven"])
        self.assertEqual(attestation["maximum_age_minutes"], 15)
        self.assertTrue(attestation["cannot_postdate_approval"])
        self.assertTrue(attestation["exact_test_organization_binding"])
        self.assertTrue(attestation["private_evidence_sha256_required"])
        self.assertTrue(
            attestation["domain_separated_attestation_digest_bound_by_approval"]
        )
        capability = billing["catalog_capability_attestation_contract"]
        self.assertTrue(capability["required_for_every_phase"])
        self.assertTrue(capability["exact_typed_write_contract_required"])
        self.assertTrue(capability["complete_normalized_readback_contract_required"])
        self.assertTrue(capability["mark_inactive_rollback_contract_required"])
        self.assertTrue(capability["effective_test_tenant_access_required"])
        self.assertTrue(capability["separately_supplied_reviewed_registry_required"])
        self.assertTrue(
            capability["request_and_response_digests_must_match_registry"]
        )
        self.assertFalse(capability["packet_supplied_contract_hashes_are_authority"])
        self.assertFalse(
            capability["environment_supplied_contract_hashes_are_authority"]
        )
        self.assertEqual(
            capability["registry_environment_variable"],
            "BILLING_CATALOG_CAPABILITY_CONTRACTS_JSON",
        )
        self.assertEqual(
            capability["committed_authority_path"],
            "src/zoho-catalyst/crm-billing-orchestrator/config/"
            "billing-catalog-capability-authority.json",
        )
        self.assertEqual(
            capability["committed_authority_status"],
            "blocked_no_reviewed_installed_contract",
        )
        self.assertFalse(capability["committed_authority_executable"])
        self.assertTrue(
            capability["caller_registry_digest_must_match_committed_authority"]
        )
        self.assertFalse(capability["current_registry_configured"])
        self.assertFalse(capability["current_installed_connector_satisfies_contract"])
        self.assertTrue(capability["mutation_validator_currently_fails_closed"])
        capability_authority = json.loads((
            ROOT
            / "src/zoho-catalyst/crm-billing-orchestrator/config/"
            "billing-catalog-capability-authority.json"
        ).read_text(encoding="utf-8"))
        self.assertEqual(capability_authority, {
            "schemaVersion": 1,
            "status": "blocked_no_reviewed_installed_contract",
            "executable": False,
            "registrySha256": None,
        })
        consumption = billing["catalog_approval_consumption_contract"]
        self.assertTrue(consumption["single_use_field_is_declarative"])
        self.assertTrue(consumption["domain_separated_approval_digest_returned"])
        self.assertTrue(consumption["stable_operation_authorization_digest_returned"])
        self.assertFalse(
            consumption[
                "operation_authorization_digest_depends_on_approval_timestamps"
            ]
        )
        self.assertTrue(
            consumption[
                "consumption_digest_binds_stable_authority_id_and_exact_packet"
            ]
        )
        self.assertTrue(
            consumption["stable_authority_id_returned_for_unique_ledger_key"]
        )
        self.assertTrue(
            consumption["durable_atomic_executor_consumption_required_before_write"]
        )
        self.assertTrue(
            consumption[
                "executor_must_use_unique_authority_id_and_reject_consumption_digest_conflicts"
            ]
        )
        self.assertFalse(
            consumption["separate_untracked_packet_digest_required_for_consumption"]
        )
        self.assertTrue(
            consumption[
                "consumption_retained_after_success_partial_timeout_or_ambiguity"
            ]
        )
        self.assertFalse(consumption["durable_executor_currently_proven"])
        self.assertFalse(
            consumption["validator_success_alone_is_mutation_authorization"]
        )
        contract_markdown = CONTRACT_MARKDOWN_PATH.read_text(encoding="utf-8")
        for phrase in (
            "private catalog contract is schema v4",
            "registry must match the digest in the committed capability-authority record",
            "`singleUse: true` remains declarative",
            "atomically claim the ID as its UNIQUE authority key",
        ):
            self.assertIn(phrase, contract_markdown)
        zoho_readme = ZOHO_README_PATH.read_text(encoding="utf-8")
        for phrase in (
            "Schema v4 requires fresh complete paginated target absence",
            "registry must match the digest in the committed capability-authority record",
            "`singleUse` remains declarative",
            "atomically claim the stable operation-authorization ID as a UNIQUE key",
        ):
            self.assertIn(phrase, zoho_readme)
        self.assertEqual(billing["usage_addon_contract"], {
            "type": "recurring",
            "is_usage_supported": True,
            "usage_tracking_mode": "usage_billing",
            "pricing_scheme": "unit",
            "interval": 1,
            "interval_unit": "months",
            "unit": "minute",
            "price_bracket_count": 1,
            "price_bracket_start_quantity": 1,
        })
        automation = json.loads((
            ROOT / "src/zoho-crm/free-revenue-leak-test/config/automation-contract.json"
        ).read_text(encoding="utf-8"))
        billing_evidence = automation["blueprint"]["external_evidence_contracts"][
            "billing-closed-won-reconciliation-v1"
        ]
        exact_billing_readback_fields = [
            "customer_id",
        ...
        exact_billing_invariants = {
            "required_currency": "USD",
            "required_usage_addon_unit": "minute",
            "immutable_subscription_status_map": {
                "future": "Scheduled",
                "live": "Active",
            },
            "closed_won_required_provider_subscription_status": "live",
            "closed_won_required_crm_subscription_status": "Active",
            "billing_organization_binding": {
            ...
            "exact_billing_readback_fields": exact_billing_readback_fields,
        }
        for field, expected in exact_billing_invariants.items():
            with self.subTest(billing_invariant=field):
                self.assertEqual(expected, billing[field])
                self.assertEqual(expected, billing_evidence[field])
        self.assertEqual(
            billing["fingerprint_fresh_crm_bindings"],
            billing_evidence["fingerprint_fresh_crm_bindings"],
        )
        self.assertEqual(
            billing["exact_crm_readback_fields"],
            billing_evidence["exact_crm_readback_fields"],
        )
        self.assertEqual(
            {entry["api_name"] for entry in billing["fingerprint_fresh_crm_bindings"]},
            {
                "Account_Name",
                "Results_Review_At",
                "Plan",
        ...
        self.assertEqual(
            blueprint_gate["status"],
            "source_candidate_installation_and_readback_pending",
        )
        ...
        self.assertTrue(blueprint_gate["external_evidence_validator_in_repository"])
        self.assertEqual(
            blueprint_gate["external_evidence_validator_path"],
            "src/zoho-crm/free-revenue-leak-test/validators/external_evidence.py",
        )
        self.assertFalse(blueprint_gate["external_evidence_validator_live_caller_wired"])
        self.assertTrue(
            blueprint_gate["approval_activation_current_poststate_version_bound"]
        )
        self.assertTrue(
            blueprint_gate["approval_activation_signed_intents_bind_exact_current_deal"]
        )
        self.assertTrue(
            blueprint_gate[
                "activation_referenced_approval_scope_binds_same_deal_and_event"
            ]
        )
        self.assertTrue(
            blueprint_gate[
                "activation_chain_uses_pairwise_distinct_approval_activation_and_operator_secrets"
            ]
        )
        evidence_contracts = automation["blueprint"]["external_evidence_contracts"]
        approval_contract = evidence_contracts["internal-approval-receipt-v1"]
        activation_contract = evidence_contracts["route-activation-readback-v1"]
        for contract in (approval_contract, activation_contract):
            intent_schema = contract["cryptographic_boundary"]["intent_schema"]
            self.assertIn("deal_id", intent_schema["canonical_fields"])
            self.assertEqual(intent_schema["deal_id_pattern"], "^[1-9][0-9]{7,29}$")
            self.assertIn("current private CRM context deal_id", intent_schema["deal_id_binding"])
        ...
        self.assertEqual(
            ...
            approval_contract["one_time_consumption"]["durable_unique_scope"],
        )
        self.assertIn(
            "same_validated_current_crm_deal_id",
            referenced_approval["same_deal_binding"]["operator"],
        )
        self.assertIn(
            "activation_intent_approval_event_key",
            referenced_approval["same_approval_event_binding"]["operator"],
        )
        ...
        self.assertTrue(activation_crypto["runtime_secrets_pairwise_distinct_required"])
        self.assertEqual(
        ...
        self.assertTrue(
            blueprint_gate[
                "activation_route_and_prestate_observation_exactly_equal"
            ]
        )
        self.assertTrue(
            blueprint_gate[
                "terminal_current_deployment_configuration_readback_required"
            ]
        )
        self.assertTrue(
            blueprint_gate["contract_specific_private_identifier_grammars_enforced"]
        )
        self.assertTrue(
            blueprint_gate["route_inactive_context_deal_id_grammar_enforced"]
        )
        self.assertEqual(
            evidence_contracts["route-inactive-readback-v1"]["context_deal_id_pattern"],
            "^[1-9][0-9]{7,29}$",
        )
        self.assertTrue(
            blueprint_gate["route_inactive_nullable_bindings_use_tagged_encoding"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            closed_won["current_crm_bindings"][:2],
            [
                "Account_Name current lookup record ID",
                "Results_Review_At exact current CRM string (instant-normalized only for private-context comparison)",
            ],
        )
        self.assertEqual(
            closed_won["provider_readback_max_age_at_evidence_seconds"], 300
        )
        self.assertTrue(
            closed_won[
                "provider_and_crm_readbacks_independently_ordered_after_operation"
            ]
        )
        self.assertEqual(
            closed_won["blueprint_validator_status"], "implemented_repository_only"
        )
    ...
    def test_terminal_report_handoff_is_automatic_revision_safe_and_human_reviewed(self):
        ...
        self.assertIn(
            "fresh current Deal Deployment_Record_ID and Configuration_Version",
            handoff["manual_transition_current_binding_gate"],
        )
    ...
    def test_analytics_dashboards_and_key_rotation_are_release_gates(self):
        ...
        shared_runtime_consumers = {
            "revenue_desk_call_gateway": "gateway",
            "revenue_desk_route_control": "route_control",
            "revenue_desk_call_worker": "worker",
        }
        for function_name, registry_reference in registries.items():
            ...
            component_references = [
                ...
                and (
                    function_name not in shared_runtime_consumers
                    or variable.get("consumer")
                    == shared_runtime_consumers[function_name]
                )
            ]
        ...
        self.assertFalse(rotation["expands_exact_seven_revenue_desk_functions"])
        ...
        form1_rotation = next(
            ...
            if entry["consumers"] == ["revenue_leak_test_request_form"]
        )
        self.assertEqual(
            form1_rotation["rule"],
        ...
        self.assertTrue(all(
            not entry["included_in_exact_seven_revenue_desk_functions"]
