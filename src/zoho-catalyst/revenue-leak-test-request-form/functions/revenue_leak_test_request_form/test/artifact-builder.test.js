...
const repositoryControllerPath = "src/zoho-catalyst/revenue-leak-test-request-form";
...
function createFixture(testContext, mutateBeforeCommit) {
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sylvara-revenue-leak-test-request-form-builder-test-"));
  ...
  const sourceRevision = path.join(
    copiedController,
    "functions/revenue_leak_test_request_form/lib/source-revision.js",
  );
  const environment = {
    ...process.env,
...
test("rejects a linked Git tree entry", (testContext) => {
  const fixture = createFixture(testContext, ({ copiedController }) => {
    fs.writeFileSync(
      path.join(copiedController, "functions/revenue_leak_test_request_form/lib/escape.js"),
      "../../outside",
      "utf8",
    );
  });
  const linkRelative = `${repositoryControllerPath}/functions/revenue_leak_test_request_form/lib/escape.js`;
...
test("rejects a relative dependency that escapes the bundled function", (testContext) => {
  const fixture = createFixture(testContext, ({ copiedController }) => {
    fs.appendFileSync(
      path.join(copiedController, "functions/revenue_leak_test_request_form/index.js"),
      "\nrequire('../outside');\n",
    );
  });
  const result = runBuilder(fixture);
