...
function validateJsonPost(request, allowedPaths) {
  ...
  const encoding = String(getHeader(request, "content-encoding") ?? "identity").toLowerCase();
...
async function readRawBody(request, { maximumBytes, timeoutMs }) {
  ...
  if (!request || typeof request.on !== "function") {
    throw new HttpBoundaryError("Raw body stream is unavailable", { publicCode: "body_unavailable" });
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const onError = () => finish(reject, new HttpBoundaryError("Body stream failed", {
      publicCode: "body_unavailable",
    }));
    const onAborted = () => finish(reject, new HttpBoundaryError("Body stream aborted", {
      publicCode: "body_unavailable",
    }));
    const timer = setTimeout(() => {
      finish(reject, new HttpBoundaryError("Body stream timed out", {
        status: 408,
...
function parseJsonObject(rawBody) {
  if (!rawBody.length) throw new HttpBoundaryError("Body is required", { publicCode: "body_required" });
  let value;
  try {
...
async function readResponseJson(response, maximumBytes, { ambiguous = false } = {}) {
  ...
  if (declared && (!/^[0-9]+$/.test(declared) || Number(declared) > maximumBytes)) {
    throw new HttpBoundaryError("Dependency response exceeded its bound", {
      status: 503,
  ...
  try {
    ...
    throw new HttpBoundaryError("Dependency response was not bounded JSON", {
      status: 503,
      publicCode: "dependency_failed",
      ambiguous,
    });
...
async function requestJson(url, options, boundary, fetchImpl = globalThis.fetch) {
  ...
  try {
    ...
    throw new HttpBoundaryError("Dependency request failed", {
      status: 503,
      publicCode: "dependency_failed",
      ambiguous: Boolean(boundary.sideEffecting),
    });
