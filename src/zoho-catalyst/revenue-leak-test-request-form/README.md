...
**Development status, updated 2026-08-28:** Fresh release readback proves that `revenue_leak_test_request_form` is installed at revision `288a93c7773acaf82fab277702e6b4e3d7354564` with Node 24, 256 MB, its prior 30-variable private map, and Catalyst archive pullback parity. That installed revision predates this full containment correction and is not an activation or rollback candidate. The corrected source reduces the registry to ten route/source/project variables, rejects every superseded token, Form URL, CRM, Connection, Data Store, session, body, and outbound variable, and removes the Catalyst SDK dependency and all corresponding runtime modules. Earlier readback confirms that `RevenueLeakTestRequestFormSessions` is empty and that `Form1AssistedSessions` is preserved with eight rows; neither table is bound by corrected source. Both callers and all current and predecessor assisted routes must remain disabled. No live cleanup or deployment is authorized by this repository change.**
...
- The CRM button and Forms Prefill Webhook remain unbound. Both current and predecessor route generations remain disabled.
...
1. Keep API Gateway, both current assisted routes, both predecessor assisted routes, the CRM button, and the Forms Prefill Webhook disabled or unbound. Do not use a prior artifact as rollback.
2. Before any separately approved containment deployment, read back the exact function revision, full variable-name set, route generations, caller bindings, Connection grants, and both session tables. Stop on drift.
3. The same packet must replace only `revenue_leak_test_request_form` with a clean stamped containment artifact and reduce its variable map exactly to `config/variables.json`. Remove Form 1-specific Connection grants from the function. Do not delete shared Connections or tables without independent dependency and ownership proof.
4. Read back exact artifact parity, the ten-name variable map, dependency-free package, both disabled route generations, both unbound callers, and zero table/Connection binding. Any ambiguity leaves both route generations disabled.
