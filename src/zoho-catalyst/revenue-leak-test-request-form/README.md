...
Historically, the local-only `Start Free-Test Request` containment control was retained until replacement Development E2E acceptance. The current `Start Free-Test Request` is the accepted Client Script launcher, formerly labeled `Open Free-Test Setup`; the local-only predecessor and its unused duplicate are retirement-scoped and must not be recreated as rollback. Historical containment evidence remains historical and does not prove the current source is installed.
...
For a separately authorized installation, install the five routes, session table, two CRM Connections, dedicated Forms prefill-handle alias and webhooks, and the `Start Free-Test Request` Client Script caller together, then read each back immediately. Preserve all historical session evidence; these installation instructions do not authorize reinstallation or another canary during legacy cleanup. Rollback keeps the failed active launcher disabled and disables its webhooks and five routes; do not recreate the retired local-only control or either retired Function, or restore unsafe pre-containment code.
