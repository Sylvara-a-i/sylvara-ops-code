...
Three live Forms prerequisites remain blocking. A connector-first 2026-08-28 preflight found no purpose-built Zoho Forms connector, so the authenticated read-only browser fallback inspected the builder and native CRM integration without saving, submitting, or reading entries. Private readback independently matched the expected business account and Forms tenant namespace without publishing either value. First, the exact native CRM contract contains 29 unique API-name destinations derived from this controller plus audit, consent, provenance, and UTM requirements; the live integration exposes six display-label mapping pairs, but the provider readback did not prove the display-label-to-API-name crosswalk or an exact missing API-name set. All 29 destinations therefore require authoritative API-name readback before cutover. Second, the public-path `Intake_Submission_ID` is a non-respondent Zoho Forms-generated alphanumeric identifier with length 10 and repetition restriction enabled, but synthetic retry/re-push and CRM readback must still prove that the same non-blank identity survives an exact safe provider retry without duplicate Lead creation. Third, every Form 1 field still requires a privately retained field-by-field dictionary covering type and alias, mandatory rule, personal classification, encryption, hidden or locked state, respondent editability, retention, and downstream mapping. Do not infer a setting from a neighboring field or apply a bulk privacy default.
...
6. After all three live Forms prerequisites above have exact private readback and approval, add one hidden or locked Zoho Forms Prefill-Webhook field with the configured alias. POST `{"token":"<field value>"}` to the prefill route, send the prefill shared header, and map only `lib/form-contract.js` outputs.
