...
Two live Forms prerequisites remain blocking. A connector-first 2026-08-28 preflight found no purpose-built Zoho Forms connector, so the authenticated read-only browser fallback inspected the builder without saving or submitting. The observed account showed the public-path `Intake_Submission_ID` as a non-respondent Zoho Forms-generated alphanumeric identifier with length 10 and repetition restriction enabled, but expected Sylvara Forms tenant binding was not independently proven. Treat that as an unbound observation, not live prerequisite closure. Fresh expected-tenant readback plus synthetic retry/re-push and CRM readback must prove that the same non-blank identity survives an exact safe provider retry without duplicate Lead creation. Separately, every Form 1 field—including audit, attribution, consent, and assisted-prefill inputs—still requires a privately reviewed field-by-field dictionary covering type and alias, mandatory rule, personal classification, encryption, hidden or locked state, respondent editability, retention, and downstream mapping. Do not infer a setting from a neighboring field or apply a bulk privacy default.
