...
const { FORM2_PREFILL_MAPPING_SAMPLE } = require("./sample");
...
function validateProbe(rawBody) {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body) ||
      ...
      !/^\s*\{\s*"prefillHandle"\s*:\s*"ZZZ_SYNTHETIC_MAPPING_ONLY"\s*\}\s*$/.test(text)) {
    // JSON.parse alone silently accepts duplicate keys. This one-purpose
    // fixture permits only the literal probe and JSON whitespace around it.
...
function createRequestListener({
  ...
  return async function requestListener(request, response) {
    try {
      ...
      // A request arriving immediately before expiry may finish afterward.
      // It must not receive a successful mapping response outside the window.
      const completedAtMs = now();
      if (!Number.isSafeInteger(completedAtMs) || completedAtMs < nowMs) {
        throw new FixtureConfigurationError("Fixture clock is invalid");
      }
      if (completedAtMs >= config.expiresAtMs) {
        throw new FixtureRequestError("Fixture has expired", {
          status: 410,
          publicCode: "fixture_expired"
        });
      }
      sendJson(response, 200, FORM2_PREFILL_MAPPING_SAMPLE);
    } catch (rawError) {
