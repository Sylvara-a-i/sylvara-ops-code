...
**Repository status: proposed Development controller. The existing three-table, 54-application-column Development baseline, table access, and two least-privilege CRM Connections were independently read back on 2026-08-14. This proposed contract adds a mandatory unique `DEAL_ISSUANCE_KEY` as the 55th application column and changes the session `LAST_OUTCOME` privacy flag; neither schema change is provisioned or read back. Function, pipeline, and API routes: not deployed. Production: code-blocked.**
...
1. **Issue:** an internal caller provides one Deal ID and an immutable UUID v4 request ID. The function derives a retry-stable 256-bit bearer token, binds its hashes and the restricted Contact, Account, and Deal IDs to one session, acquires the Deal's unique active-generation digest, conditionally marks setup access issued, then durably finalizes the session before returning the expiring Form URL.
...
- A mandatory unique, domain-separated SHA-256 digest of the Deal ID is stable across token-pepper rotation and is the atomic active-generation lock. The restricted session row already contains the bound Deal ID; the digest and every payload-derived `submitting_<fingerprint>` outcome are therefore private operational data.
...
`Setup_Form_Submission_ID` is not metadata-unique in the current CRM snapshot, so the globally namespaced Forms identity and unique Catalyst receipt are the primary duplicate boundary. If CRM nevertheless returns the documented rollback response with the Deal update failing as `DUPLICATE_DATA`, the controller treats it as a replay only after independent Contact, Account, and Deal reads match every intended update, relationship, and protected field. Any different response shape or readback enters normal rejection or reconciliation handling.

## Manual Development Setup

The Catalyst MCP catalog cannot create a project, create a function, or upload function source. The dedicated Development project, two CRM Connections, and existing 54-column table baseline have current readback evidence; the proposed 55th column, the session `LAST_OUTCOME` privacy change, GitHub pipeline, function, runtime variables, and routes do not. The reviewed Git pipeline will use the Catalyst CLI to create or update only `form2_controller` from this source; it does not rely on a nonexistent MCP source-upload action.
...
5. Before changing the Development session table, independently prove it contains zero rows; the function and routes are undeployed, so any row is unexpected. If it is not empty, stop for a reviewed migration—do not invent a default or backfill. Add `DEAL_ISSUANCE_KEY` as a private, mandatory, unique `varchar(64)` column and enable the private-data flag on session `LAST_OUTCOME`. Then read back all 55 application columns plus type, length, mandatory, unique, privacy, scope, permission, and client-access state for all three tables. Until both changes are provisioned and read back, [`config/datastore-schema.json`](config/datastore-schema.json) is proposed and deployment remains blocked.
...
- the session table is independently proven empty before the schema change, and the mandatory unique `DEAL_ISSUANCE_KEY` plus private session `LAST_OUTCOME` flag are provisioned and read back as part of the full 55-column contract;
