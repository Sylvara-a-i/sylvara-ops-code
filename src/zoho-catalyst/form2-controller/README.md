...
**Repository status: proposed Development controller. On 2026-08-22, all three Form 2 tables were provisioned in the existing Retell Catalyst Development project and the initial 54-application-column version 2 copy was independently read back. This source now proposes the 55-column version 3 schema so an immutable issuance-request digest remains a tombstone across `TOKEN_PEPPER` rotation; that migration has not been applied. On 2026-08-23, both Form 2 CRM Connections were read back in the Retell Development console as Connected with the exact approved scopes; their private link names remain outside Git, and CRM-organization binding and runtime behavior are still unverified. The copied version-2 tables and legacy Form 2 project remain live. The Retell-target function, pipeline, runtime variables, API routes, and Forms callers have not been cut over or runtime-tested. Production remains code-blocked.**
...
The Catalyst MCP catalog cannot create a project, create a function, or upload function source. The existing Retell Development project contains the three copied Form 2 version-2 tables, and their initial 54-column copy has readback evidence. The two target CRM Connections have current console evidence for their private link names, Connected status, and exact grants, but not for the bound CRM organization or runtime behavior. The 55-column version-3 session schema in this source has not been applied and requires fresh row-state proof. The copied tables and legacy Form 2 project remain live; none of this proves the Retell-target function, pipeline, variable binding, route, caller, deployment, or runtime state. The checked-in pipeline is containment-only: it retains the human approval boundary but always fails before invoking the Catalyst CLI or [`scripts/deploy-development.sh`](scripts/deploy-development.sh) until a separately reviewed native secret binding is proven.
...
5. **Version 3 migration not applied:** all three Form 2 version-2 tables were provisioned and their initial 54-column copy was read back on 2026-08-22. Re-read their names, every column property, permissions, client access, and current row state. If any session row exists, do not rename, overwrite, or backfill that table; provision a distinct version-3 table and preserve version 2 for recovery and audit. Only with fresh zero-row proof may `TOKEN_HASH` be renamed to mandatory unique private `ISSUE_REQUEST_KEY` and mandatory non-unique private `ACCESS_TOKEN_HASH` varchar(64) be added. Read back all 55 application columns; `ISSUE_REQUEST_KEY` and `DEAL_ISSUANCE_KEY` must be the only unique varchar columns. Do not upload source, deploy, or activate routes as part of schema work.
6. Set every variable name in [`functions/form2_controller/.env.example`](functions/form2_controller/.env.example) in Catalyst Development. Set `SOURCE_REVISION` to the same reviewed 40-character commit as `APPROVED_SOURCE_REVISION`. Normalize the reviewed `FORM2_PUBLIC_URL`, calculate its lowercase SHA-256, and independently review it as `APPROVED_FORM2_DESTINATION_SHA256`; a different path on the same shared host must fail. Use independent values for all four secrets, exact approved endpoints and statuses, and private allowlists and identifiers. Provider values remain bounded as documented by the registry; compressed provider JSON is reversible private encoding, not encryption. Never upload a populated environment file.
7. Keep [`scripts/deploy-development.sh`](scripts/deploy-development.sh) unreachable while the native secret-binding contract is unverified. It remains a reviewed Development-only candidate: it rejects a dirty or different checkout, verifies Node.js 24.19.0, requires Catalyst CLI 1.26.1, runs the safety scanner, proves artifact equivalence, stamps the reviewed revision, requires the exact Form destination digest to be source-bound, and limits a future invocation to `functions:form2_controller`. The destination sentinel deliberately fails deployment, and runtime input cannot replace it. A separately reviewed pipeline change plus independent approval and post-deployment readback are still required. Nothing in the current pipeline deploys tables, routes, a function, or Production.
...
- the three copied Retell Development version-2 tables are independently re-read against the full 54-column contract, including current row state, mandatory unique `TOKEN_HASH` and `DEAL_ISSUANCE_KEY`, private session `LAST_OUTCOME`, table access, scopes, permissions, and client-access denial;
- before version 3, the session table is independently proven empty; if any row exists, the version-2 table is preserved and a distinct version-3 table is provisioned instead of renaming or overwriting it;
- only after fresh zero-row proof, `TOKEN_HASH` is renamed to mandatory unique private `ISSUE_REQUEST_KEY`, mandatory non-unique private `ACCESS_TOKEN_HASH` is added, and the full 55-column version-3 contract is independently read back before source upload;
