...
3. **Submission:** Zoho Forms supplies the token, `prefillId`, its server-generated Unique ID, and the flat allowlisted Form 2 fields. The function checks the exact fingerprinted receipt before applying token expiry, claims a unique durable receipt, rejects a stale revision, consumes the prefill once, and sends one ordered CRM Composite request: Contact, Account, then Deal, with rollback on failure and `If-Unmodified-Since` on every record. It acknowledges success only after independent CRM readback, a durable succeeded receipt, and a terminal submitted session. An exact completed webhook retry remains recoverable after token expiry; a lost session-finalization response is repaired and read back before success is returned.
...
- Decision-Maker Role and Exact Job Title are coalesced server-side into one Contact `Title`: a listed role becomes the title, while `Other` requires a non-generic exact title. Client-provided title text cannot override a listed role.
...
The controller intentionally accepts the Form's conditional nulls: No-Answer Delay is absent for After Hours Only, Approved Fallback Number is absent for unnumbered destinations, and Alert Recipient Email is absent when Mobile is present. A non-active Blueprint v4 draft was read back with those three `Begin Setup and QA` inputs optional, but active v3 still requires them unconditionally. Keep every controller route disabled until v4 is activated and the active Blueprint is independently read back; repository or draft-state evidence is not enough.
...
5. The three Development Data Store tables and all 54 application columns were independently read back against [`config/datastore-schema.json`](config/datastore-schema.json) on 2026-08-14, including lengths, mandatory and unique constraints, privacy flags, scopes, permissions, and no App User access. Re-read the same contract immediately before deployment; any drift or missing unique-key constraint is an activation blocker.
6. Set every variable name in [`functions/form2_controller/.env.example`](functions/form2_controller/.env.example) in Catalyst Development. Set `SOURCE_REVISION` to the same reviewed 40-character commit as `APPROVED_SOURCE_REVISION`. Use independent random values for all four secrets, the exact US CRM V8 base URL, the exact default `forms.zohopublic.com` Form URL, all five current private setup-access status values, current private CRM picklist values, and private paths/identifiers. Custom Forms domains and non-US data-center hosts are deliberately rejected; adding one requires an exact source allowlist change and security review. Never upload a populated environment file.
7. Approve the pipeline only after its checkout exactly matches `APPROVED_SOURCE_REVISION`. Use an x86-64 Catalyst Ubuntu runner that provides Bash, the preinstalled Catalyst CLI, curl, Git, Python 3, SHA-256 tooling, tar, and xz; the script checks each prerequisite and fails before deployment rather than silently changing the runner. The checked-in [`scripts/deploy-development.sh`](scripts/deploy-development.sh) rejects a dirty or different checkout, exports the immutable Git tree, stamps its otherwise invalid source-revision sentinel with that exact commit, verifies the pinned Node.js 24.18.0 archive checksum, requires the runner's preinstalled Catalyst CLI to report exactly 1.26.1, runs the repository safety scanner and the function CI suite against the stamped artifact, then executes one US-region Development-only deploy with `--only functions:form2_controller`. Runtime configuration rejects any `SOURCE_REVISION` that is not the same lowercase 40-character commit embedded in the artifact. The script never installs a mutable CLI dependency graph at runtime. The package root contains `catalyst.json`; the function contains `catalyst-config.json`, exact Node 24 runtime metadata, and an exact-version SDK lockfile. It does not deploy tables, routes, another function, or Production.
...
5. Map the visible Title dropdown to `decisionMakerRole` and Exact Job Title to `jobTitle`. For a listed role, the controller writes that role to Contact `Title`; for `Other`, Exact Job Title is required and becomes Contact `Title`. Do not add a second CRM title field.
