...
- Keep every Retell-target Form 2 route disabled until a trusted server-verifiable step proves email OTP and CAPTCHA before Prefill can mark setup access Verified. Native Forms UI configuration alone is not server proof. SMS is outside the approved workflow and must not be configured.
