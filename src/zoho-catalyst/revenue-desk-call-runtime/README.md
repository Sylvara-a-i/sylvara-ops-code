...
Deploy `crm_billing_orchestrator` first and read back its exact source revision before deploying either call-runtime function. Prove with a synthetic, isolated Development operation that the consumer accepts a v2 null workflow-failure total and still validates a retained v1 non-null row; do not use a real Deal or Retell traffic for this compatibility proof. Keep the gateway, worker, and retry trigger dark until that consumer readback passes. This ordering is mandatory because an older v1-only consumer correctly rejects a v2 payload and would leave terminal reconciliation pending.
