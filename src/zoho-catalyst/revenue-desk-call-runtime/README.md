...
[`config/datastore-schema.json`](config/datastore-schema.json) remains an activation contract, not live authorization. Development readback confirms that the five canonical operational tables exist and are empty, but `RevenueDeskConfigurationVersions` still lacks seven reviewed columns and `AnalyticsSyncOutbox` lacks the nullable unique provider-version fence. Fresh metadata, permissions, counts, digests, samples, conflicts, and source/destination readback remain mandatory before any additive update, function binding, or migration.
