...
[`config/datastore-schema.json`](config/datastore-schema.json) remains an activation contract, not live authorization. Development readback confirms that the five canonical operational tables exist and are empty, and the exact required `RevenueDeskConfigurationVersions` application schema is now present. `AnalyticsSyncOutbox` still lacks the nullable unique provider-version fence: the approved first-party UI attempt did not create it, independent connector readback confirmed absence, the fully paginated row count remained 307 with no row mutation attempted, and the packet stopped before either canonical Job pool was created. Fresh metadata, permissions, counts, digests, samples, conflicts, source/destination readback, and a new scoped approval remain mandatory before retrying the outbox addition, creating either Job pool, binding a function, or migrating data. The sanitized execution record is [`../evidence/free-revenue-leak-test-development-packet-a-execution-2026-08-26.json`](../evidence/free-revenue-leak-test-development-packet-a-execution-2026-08-26.json).
