...
function asRawUtf8(rawBody) {
  ...
  try {
    ...
    throw new RevenueDeskError('INVALID_RAW_BODY', 'Webhook body is not valid UTF-8.', { cause: error, httpStatus: 400 });
  }
}
...
function numberLookupKey(secret, toNumber) {
  return `num_${keyedDigest(secret, 'revenue-desk-number-v1', [toNumber])}`;
}

function eventReceiptKey(secret, event, callId) {
  return `evt_${keyedDigest(secret, 'revenue-desk-event-v1', [event, callId])}`;
}

function callLookupKey(secret, callId) {
  return `call_${keyedDigest(secret, 'revenue-desk-call-v1', [callId])}`;
}

function payloadFingerprint(secret, rawBody) {
  return keyedDigest(secret, 'revenue-desk-event-payload-v1', [asRawUtf8(rawBody)]);
}

function publicCorrelationId(secret, parts) {
  return `corr_${keyedDigest(secret, 'revenue-desk-correlation-v1', parts).slice(0, 32)}`;
}
