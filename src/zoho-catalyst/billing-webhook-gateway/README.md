...
The checked-in artifact source-revision sentinel deliberately fails startup. A future reviewed immutable export must stamp the exact lowercase 40-character Git revision and the runtime `SOURCE_REVISION` must match it. No approved Billing deployment/stamping path exists yet, so this source remains non-deployable in addition to its explicit Production block.
...
6. In Catalyst Development, prove an officially supported Creator authentication/invocation path. The current candidate must return only the OAuth `Authorization` header and no parameters through the exact pinned SDK; otherwise replace it. Add a reviewed immutable export step that stamps the exact Creator destination digest without rendering credentials into shell source. Also prove hosted provenance for `x-zc-environment`. Record the results privately and add sanitized contract tests.
