...
**Repository status: `required_hardening_pending`. Deployment status: not deployed, not deployment-approved, and code-blocked from Production.**
...
The checked-in source-revision and Creator-destination sentinels deliberately fail startup. [`tools/build-development-artifact.js`](tools/build-development-artifact.js) is the component-specific build-only path: it accepts only an entirely clean checkout whose exact `HEAD` equals the approved lowercase 40-character revision, exports committed runtime blobs into an outside-repository Catalyst project, transforms them into exactly one Advanced I/O function named `sylvara_client_portal_hmac_gateway_function`, and stamps both immutable values only in that artifact. The runtime `SOURCE_REVISION` must equal the stamped revision. The builder never installs, deploys, or modifies the checkout, and its deterministic manifest deliberately omits the private Creator destination value and the stamped file's linkable digest.

The supplied export remains outside GitHub. This package is not a mechanically redacted copy and is not evidence of the code currently running in Catalyst. Repository tests establish only local behavior against synthetic fixtures. A successful build is packaging evidence, not deployment, route ownership, live-source parity, or activation authority.

## Sanitized Live-Audit Evidence

[`evidence/sanitized-live-audit-2026-08-25.json`](evidence/sanitized-live-audit-2026-08-25.json) records the bounded read-only audit without private identifiers, endpoints, hashes, secrets, payloads, or raw source. The active Billing subscription webhook target exactly matched the Development route path. No delivery rows, status codes, or timestamps were visible in either the three-month or one-year history query; absence in those views is not proof that no delivery or execution occurred.

Private full-source scans covered all three observed live source revisions: Development snake case, Production snake case, and Production camel case. Each directly referenced Billing, Creator, and raw OAuth refresh material; none directly referenced CRM, Books, Analytics, or the website, and none used Catalyst Connections. Creator Custom API inventory remains unverified because neither Creator UI nor an authorized Creator MCP capability was available. The active dependency proves only that retention is required. Classification remains `required_hardening_pending` until Creator Custom API/auth proof, immutable Development deployment/readback, route proof, duplicate-removal proof, webhook and fingerprint rekeying, Connection-grant rotation, old raw-OAuth revocation, and final readback all pass under separately scoped approvals.
...
The current authorization adapter attempts to retrieve exactly one `Authorization` header and no query parameters through Catalyst Connections. This is not yet an official, proven Creator connection contract: the published Node SDK credential method is documented for Default Services, and Creator is not in the current Default Services list. A real Catalyst Development fixture must prove whether an approved Custom Service/Connection can supply credentials through this exact pinned-SDK method, or the adapter must be replaced with an officially supported invocation path. The Creator Custom API itself must require OAuth2 with `Zohocreator.customapi.EXECUTE`. Runtime configuration cannot authorize a credential destination: source pins the US Custom API origin to `https://www.zohoapis.com`, and `creator` mode also requires the normalized full URL to match the SHA-256 digest stamped into the reviewed artifact. The checked-in sentinel deliberately fails startup; the artifact builder now provides the reviewed build-only stamping boundary without disclosing that digest. Until the authentication contract, Custom API inventory, and Development readback are proven, Creator delivery remains unsupported for activation. The raw client ID, client secret, refresh token, token-refresh endpoint, and runtime host-allowlist variables from the historical export remain retired.
...
6. In Catalyst Development, prove an officially supported Creator authentication/invocation path. The current candidate must return only the OAuth `Authorization` header and no parameters through the exact pinned SDK; otherwise replace it. Build from a clean exact reviewed `HEAD` with the component artifact builder and supply the pre-reviewed Creator destination digest only as private build input. Also prove hosted provenance for `x-zc-environment`. Record the results privately and add sanitized contract tests.
...
12. Privately retain the returned outside-repository artifact and deterministic manifest, independently confirm the exact function target and stamped source revision without publishing the Creator binding, test containment and rollback, and obtain separate approval before any deployment or live Billing change.
...
After CI passes and only from a clean checkout at the exact approved revision, create a build-only artifact with private process-environment inputs:

```powershell
$env:APPROVED_SOURCE_REVISION = "<exact-reviewed-lowercase-40-character-commit>"
$env:APPROVED_CREATOR_DESTINATION_SHA256 = "<pre-reviewed-private-lowercase-64-character-digest>"
npm run artifact:build
```

Use `npm run artifact:build -- --output <absolute-new-outside-repository-directory>` when an explicit destination is required. The output is a single-target Catalyst project plus `release-manifest.json`; no command in this package deploys it. The dependency is exact-version pinned and the lockfile is committed. Tests, `.env` files, documentation, source-only tools, `node_modules/`, deployment identifiers, payloads, and logs are excluded from the artifact.
