...
test("the concrete Forms desired state keeps Form 1 idempotent and both forms SMS-free", () => {
  ...
  const form1 = manifest.forms.find((form) => form.logical_name === "FORM1_FREE_REVENUE_LEAK_REQUEST");
...
test("the package and Catalyst manifests name one Node 24 Advanced I/O target", () => {
  ...
  assert.deepEqual(catalyst.functions.targets, ["form1_assisted_controller"]);
