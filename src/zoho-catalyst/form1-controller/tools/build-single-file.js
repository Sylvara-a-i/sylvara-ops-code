...
const CONTROLLER_SUBPATH = "src/zoho-catalyst/form1-controller";
const FUNCTION_SUBPATH = `${CONTROLLER_SUBPATH}/functions/form1_assisted_controller`;
...
function findRepositoryRoot() {
  ...
  if (
    ...
    fail("the Form 1 controller is not at its approved repository path");
  }
...
function exportApprovedFunction(repositoryRoot, approvedRevision, exportRoot) {
  ...
  const functionRoot = path.join(exportRoot, "form1_assisted_controller");
...
function createIsolatedRoot() {
  ...
  const root = fs.mkdtempSync(path.join(tempParent, "sylvara-form1-artifact-"));
  fs.chmodSync(root, 0o700);
  const realRoot = fs.realpathSync.native(root);
  if (!isWithin(tempParent, realRoot) || path.basename(realRoot).startsWith("sylvara-form1-artifact-") === false) {
...
function removeIsolatedRoot(root, tempParent) {
  if (
    ...
    path.basename(root).startsWith("sylvara-form1-artifact-") === false
  ) {
