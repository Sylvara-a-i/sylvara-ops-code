...
Keep `ANALYTICS_SYNC_MODE=disabled` until the full gate below passes. The 2026-08-24 readback established the retained row counts: `AnalyticsSyncOutbox=307`, `AnalyticsSyncCheckpoints=10`, `ClientDailyMetrics=10`, `ReportRuns=1`, `Calls=13`, and `FreeTestCalls=30`. Packet A then proved the exact required checkpoint application columns, the outbox's 71-column count, and the nullable-unique `OUTBOX_KEY` contract without rewriting a retained row. It did not prove the full outbox application schema. Fresh columns and constraints, row semantics, producer/consumer lineage, normalized keys, target matches, and watermarks still require private pre-activation readback.
