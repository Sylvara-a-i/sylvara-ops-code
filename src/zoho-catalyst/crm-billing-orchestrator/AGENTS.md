...
- Resolve Billing customers through the exact `zcrm_account_id` reference endpoint and native CRM Account import endpoint by default. The only exception is the explicit `test_direct_customer` Development gate: immediately attest the configured Billing organization through Zoho's organization API as `mode=test` and Billing-only, create only the deterministic synthetic customer shape, and authoritatively re-read it. Never send CRM Account data or use this exception in a live/joined organization.
