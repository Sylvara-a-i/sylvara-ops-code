...
- Set `ENABLE_PAID_SUBSCRIPTION_PREPARATION=false` for evaluation-only Development. While disabled, an empty paid-plan map is permitted and the paid action rejects before any CRM or Billing mutation. Enable it only after a dedicated test catalog and private allowlist are verified.
...
1. Create a dedicated Catalyst Development project, function, Data Store table, four least-privilege Connections, and one exact API Gateway route. Keep Production untouched.
