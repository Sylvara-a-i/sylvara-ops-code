...
function readRawBody(request, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(reject, new FreeTestError(
    ...
    const onData = (chunk) => {
      ...
      if (length > maximumBytes) {
        finish(reject, new FreeTestError('REQUEST_TOO_LARGE', 'Request body exceeds the configured limit.', { httpStatus: 413 }));
    ...
    const onAborted = () => finish(reject, new FreeTestError('REQUEST_ABORTED', 'Request was aborted.', { httpStatus: 400 }));
    const onError = (error) => finish(reject, new FreeTestError(
...
function parseJson(rawBody) {
  try {
    ...
    throw new FreeTestError('INVALID_JSON', 'Request body must be valid JSON.', { cause: error, httpStatus: 400 });
  }
}
