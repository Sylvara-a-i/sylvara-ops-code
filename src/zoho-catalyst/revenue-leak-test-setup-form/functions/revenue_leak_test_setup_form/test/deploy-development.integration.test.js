...
function createFixture(testContext, scenario) {
  ...
  const copiedController = path.join(repo, "src/zoho-catalyst/revenue-leak-test-setup-form");
  ...
  const copiedFormDestination = path.join(
    copiedController,
    "functions/revenue_leak_test_setup_form/lib/form-destination.js",
  );
  ...
  const canaryRelative = "src/zoho-catalyst/revenue-leak-test-setup-form/functions/revenue_leak_test_setup_form/IGNORED_CHECKOUT_CANARY";
  ...
  const replacementCanaryRelative =
    "src/zoho-catalyst/revenue-leak-test-setup-form/functions/revenue_leak_test_setup_form/REPLACEMENT_EXPORT_CANARY";
  ...
  writeExecutable(path.join(stubBin, "catalyst"), [
    ...
    "function_root=\"$PWD/functions/revenue_leak_test_setup_form\"",
    "[[ \"$PWD\" == */deploy-export/src/zoho-catalyst/revenue-leak-test-setup-form ]]",
    ...
    "checkout=\"$(\"$real_node\" -e 'process.stdout.write(require(process.argv[1]).ARTIFACT_SOURCE_REVISION)' \"$repo_root/src/zoho-catalyst/revenue-leak-test-setup-form/functions/revenue_leak_test_setup_form/lib/source-revision.js\")\"",
    ...
    "checkout_destination=\"$(\"$real_node\" -e 'process.stdout.write(require(process.argv[1]).ARTIFACT_FORM_DESTINATION_SHA256)' \"$repo_root/src/zoho-catalyst/revenue-leak-test-setup-form/functions/revenue_leak_test_setup_form/lib/form-destination.js\")\"",
...
function assertCheckoutUnchanged(fixture, { expectStartupProbes = true } = {}) {
  ...
  const revisionSource = fs.readFileSync(
    path.join(fixture.copiedController, "functions/revenue_leak_test_setup_form/lib/source-revision.js"),
    "utf8",
  );
  assert.match(revisionSource, /__SYLVARA_UNSTAMPED_SOURCE_REVISION__/);
  const destinationSource = fs.readFileSync(
    path.join(fixture.copiedController, "functions/revenue_leak_test_setup_form/lib/form-destination.js"),
    "utf8",
  );
