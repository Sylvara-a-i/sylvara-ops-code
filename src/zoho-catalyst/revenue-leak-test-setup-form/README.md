...
- Zoho CRM owns Contact, Account, Deal, relationship, approved-route, test-control, and delivery state. The Journey-core profile does not depend on the deferred Form 2 workflow or full Blueprint as a security or persistence authority.
...
One Node.js 24 Advanced I/O function accepts six exact private runtime paths. The browser-facing `FORM2_ACCESS_PUBLIC_URL` is the corresponding exact Development API Gateway source URL; it is a distinct provider identifier and must not be replaced with `FORM2_ACCESS_PATH` or a direct `/server/...` target URL.

1. **Issue (`POST`):** an administrator-restricted caller provides one Deal ID and an immutable UUID v4 request ID under the exact machine-readable [`config/issue-caller-contract.json`](config/issue-caller-contract.json). The function requires the authoritative Deal to remain in Pipeline `Revenue Desk Sales`, Stage `Setup and Authorization`, and to contain that exact persisted `Setup_Access_Issue_Request_ID`; it then derives a retry-stable bearer token, binds its HMAC and restricted CRM context to one durable session, conditionally marks setup access issued, and returns the Catalyst access URL with the token only in its URL fragment.
...
6. **Submission (`POST`):** Zoho Forms supplies `prefillId`, `configurationRevision`, the server-generated `submissionId`, and allowlisted client fields—never the setup token or prefill handle. The controller claims one durable receipt, rejects a stale or mismatched revision, consumes the prefill once, and sends one ordered rollback-enabled CRM Composite request: Contact, Account, then Deal. The Journey-core write suppresses CRM workflow triggers, persists `Setup_Access_Status = Submitted` directly, and succeeds only after independent CRM readback and a durable succeeded receipt.
...
- Active Development requires the exact `ZOHO_CATALYST_ZCQL_PARSER=V2` provider binding so every conditional durable state transition is parsed under the ZCQL version used by the controller.
...
- Current call handling, requested route, and approved route are display-only snapshots. The assigned test phone number is omitted from Form 2 and preserved as Setup/QA-owned state. Pipeline, Stage, setup issue identity, test duration, call limit, scope version, entry offer, intake provenance, and request notes remain server-owned and are independently preserved during CRM readback.
- The client cannot submit CRM IDs, server timestamps, form versions, scope versions, test limits, or arbitrary fields.
- The canonical two confirmations must be literal `true`. After route/header authentication, bounded JSON parsing, and the exact 36-key check, the submission boundary decodes only exact lowercase `"true"`/`"false"` strings for `authorizedRepresentativeConfirmed` and `testScopeAccepted` in a separate validation copy; native booleans remain unchanged. No trimming, case folding, truthiness, other field coercion, or missing-key repair is permitted. Strict business validation still rejects false or absent affirmations. Fingerprinting retains the original field types: an exact string retry is idempotent, but a string/boolean representation change under the same submission ID remains a conflict. Their timestamps and the form version are generated by the controller. `Authorized_Representative_Confirmed` and `Authority_Confirmed_At` record respondent attestation only; they are not a signature, signed-authorization evidence, or go-live approval.
...
`FORM2_PROOF_MAX_ATTEMPTS` defaults to ten code-entry attempts for newly reserved proofs; explicit integer overrides remain bounded to 2–10. Nine incorrect comparisons leave the tenth entry available. A tenth incorrect comparison fails the proof, and later entries cannot reopen it. A correct tenth entry verifies with `ATTEMPT_COUNT = 9`, because that counter records failed comparisons only. The independent `MAX_VERIFICATION_ATTEMPTS` default of three governs post-proof prefill preparation, not OTP guesses. Email sends remain capped at the separately reserved send ceiling, and the OTP lifetime is unchanged.

An immutable Development deployment is required before the new default takes effect. Read back only the nonsecret attempt override to confirm an older explicit value does not mask the default. Existing proof rows retain their reserved `MAX_ATTEMPTS`, counters, and timestamps, including after process restart or exact reservation replay; never rewrite them to obtain more attempts. A deployment/revision change is not a new canary or email-send allocation. Rollback restores the prior reviewed artifact and configuration without resetting proofs, restoring exposed credentials, or removing evidence. This policy change increases bounded code-guess opportunities but does not extend expiry, add delivery, change CRM ownership, or permit provider activation.
...
- the exact privately configured submitted setup-access status; and
- the server-owned `Configuration_Version` evidence pointer.

After the durable submission claim exists, the controller derives `Configuration_Version` exactly as `form2cfgv1:<Form2SubmissionsV3 ROWID>:<40-character receipt source revision>`. The revision comes from the durable receipt rather than the currently running artifact, so an exact provider replay remains bound to its original evidence after a later source deployment. This bounded immutable reference points to restricted Form 2 evidence; it is not telephony deployment, configuration approval, or activation evidence. The Deal prestate must be blank or already contain that exact reference. `Deployment_Record_ID` is never written and is independently preserved during readback. Exact duplicate reconciliation accepts only the original submitted state or the explicitly modeled Journey-core approved-inactive and stopped states; unrelated stage, issue-identity, deployment, or control-state drift fails closed.

It deliberately does not update `Pipeline`, `Stage`, `Setup_Access_Issue_Request_ID`, `Deployment_Record_ID`, `Test_Duration_Days`, `Test_Call_Limit`, or `Test_Scope_Version`. The authenticated Journey launcher owns initialization; this controller requires and preserves those authoritative values. The Form 2 workflow remains inactive, and its ancillary internal review task is deferred rather than duplicated in Catalyst.
...
- every Journey-core CRM write carries an empty trigger list, and a failed middle subrequest rolls back all three writes without invoking a workflow or Blueprint;
- a successful Composite directly persists Submitted state, and independent readback matches every updated and preserved field, including Pipeline, Stage, and setup issue identity;
