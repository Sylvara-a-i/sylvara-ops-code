...
IDENTIFIERS = ["client_reference", "deployment_reference", "configuration_reference"]
...
def gate_cases(contract: dict[str, Any], coverage: dict[str, Any]) -> list[dict[str, Any]]:
    base = {"coverage_mode": "AfterHoursOnly", "resolver_state": "Resolved", **dict(zip(IDENTIFIERS, ("synthetic_client", "synthetic_deployment", "synthetic_configuration"))), "engagement_type": "free_test", "capability_profile": "call_gap_monitor_v1"}
    ...
    cases += [(f"invalid_resolver_{index}", {**base, "resolver_state": value}, False) for index, value in enumerate([None, "", " ", "Degraded", "Unknown"])]
    ...
    extras = [("resolver_lowercase", "resolver_state", "resolved"), ("resolver_padded", "resolver_state", " Resolved"), ("resolver_partial", "resolver_state", "Resolve"), ("engagement_upper", "engagement_type", "FREE_TEST"), ("engagement_padded", "engagement_type", "free_test "), ("capability_partial", "capability_profile", "call_gap_monitor"), ("capability_padded", "capability_profile", " call_gap_monitor_v1")]
...
def business_state_report(contract: dict[str, Any], coverage: dict[str, Any]) -> dict[str, Any]:
    ...
    config_base = {"coverage_mode": "AfterHoursOnly", "resolver_state": "Resolved", **dict(zip(IDENTIFIERS, ("synthetic_client", "synthetic_deployment", "synthetic_configuration"))), "engagement_type": "free_test", "capability_profile": "call_gap_monitor_v1"}
    ...
    for reference, (label, value) in itertools.product(IDENTIFIERS, invalid_values):
        ...
        if not ok and len(failures) < 10: failures.append({"configuration_reference": reference, "invalid_class": label, "expected": "terminal.configuration_unavailable", "actual": "continue" if actual_valid else "terminal.configuration_unavailable"})
...
def metamorphic_report(contract: dict[str, Any], coverage: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, value in enumerate([None, "", " ", "unknown", "AfterHours", "afterhoursonly"]):
        config = {"coverage_mode": value, "resolver_state": "Resolved", **dict(zip(IDENTIFIERS, ("synthetic_client", "synthetic_deployment", "synthetic_configuration"))), "engagement_type": "free_test", "capability_profile": "call_gap_monitor_v1"}; add("malformed_gate_fails_closed", str(index), not gate_valid(config, contract, coverage))
    valid_config = {"coverage_mode": "AfterHoursOnly", "resolver_state": "Resolved", **dict(zip(IDENTIFIERS, ("synthetic_client", "synthetic_deployment", "synthetic_configuration"))), "engagement_type": "free_test", "capability_profile": "call_gap_monitor_v1"}
