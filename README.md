...
The canonical public boundary is [`docs/product/README.md`](docs/product/README.md), supported by [ADR 0002](docs/adr/0002-managed-home-service-receptionist-product-boundary.md). [ADR 0003](docs/adr/0003-initial-after-hours-service-request-workflow.md) selects `after-hours-new-residential-service-request-v1` for offline synthetic validation. [ADR 0004](docs/adr/0004-retell-catalyst-crm-analytics-integration-boundary.md) fixes the compatible Retell, Catalyst, CRM, customer-system, and Analytics ownership boundary. [ADR 0006](docs/adr/0006-shared-seven-day-monitor-with-client-number-isolation.md) is authoritative for the free test: one shared agent, one dedicated Retell number and immutable Catalyst deployment/configuration per active client, exact fail-closed admission, and a separate Revenue Desk only after an approved conversion. Those documents do not establish a live service, customer deployment, telephone route, current pricing, or customer-facing approval.
...
| Free-test deployment ownership, configuration versions, admission, call/outcome, notification, and reporting outbox | Zoho Catalyst in the approved environment; Development runtime/source parity remains unproven |
...
- [`docs/adr/0006-shared-seven-day-monitor-with-client-number-isolation.md`](docs/adr/0006-shared-seven-day-monitor-with-client-number-isolation.md) defines the exact seven-field configuration gate, deterministic seven-day/25-call admission, immutable number/configuration ownership, post-call resolution priority, durable Catalyst notification, and no-degraded-intake Configuration Unavailable path.
...
- [`docs/runbooks/shared-seven-day-monitor-number-routing.md`](docs/runbooks/shared-seven-day-monitor-number-routing.md) defines synthetic client setup, exact resolution/admission, Configuration Unavailable, post-call ownership, notification/Analytics lifecycle, 30 minimum scenarios, number reassignment, correlation tracing, later real-client gates, containment, and rollback.
...
- [`src/zoho-catalyst/retell-free-test/catalyst.json`](src/zoho-catalyst/retell-free-test/catalyst.json) anchors the Development-only free-test package. Its component-owned variable registry rejects missing configuration and Production mode; synthetic notification/Analytics and disabled CRM modes do not send or mutate external systems.
