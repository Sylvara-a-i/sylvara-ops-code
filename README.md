...
- [`src/zoho-catalyst/retell-free-test/catalyst.json`](src/zoho-catalyst/retell-free-test/catalyst.json) anchors the Development-only free-test package and its separate retry Job target. Its component-owned configuration rejects missing values and Production mode; notification defaults and returns to `dry_run`, while one tightly controlled `send_development` proof is required before internal-phone readiness. CRM is disabled and Analytics is outside the MVP.
