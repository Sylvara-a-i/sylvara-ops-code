...
The canonical public boundary is [`docs/product/README.md`](docs/product/README.md), supported by [ADR 0002](docs/adr/0002-managed-home-service-receptionist-product-boundary.md). [ADR 0003](docs/adr/0003-initial-after-hours-service-request-workflow.md) selects `after-hours-new-residential-service-request-v1` for offline synthetic validation. [ADR 0004](docs/adr/0004-retell-catalyst-crm-analytics-integration-boundary.md) fixes the proposed Retell, Catalyst, CRM, customer-system, and Analytics ownership and data-flow boundary without authorizing a live call or deployment. [ADR 0006](docs/adr/0006-shared-seven-day-monitor-with-client-number-isolation.md) supersedes the prior client-specific monitor-clone decision and selects one shared Seven-Day Monitor, one dedicated Retell number and deployment per active test client, and one dedicated Revenue Desk agent per converted client. Those documents govern product scope, progressive rollout, explicit non-goals, evidence gates, and deferred verticals. They do not establish a live service, current pricing, a customer deployment, or approval for customer-facing publication.
...
- [`docs/adr/0006-shared-seven-day-monitor-with-client-number-isolation.md`](docs/adr/0006-shared-seven-day-monitor-with-client-number-isolation.md) defines the current proposed agent and number topology: one shared low-capability monitor, one dedicated Retell number and deployment per test client, one shared inbound resolver, separate client-specific Revenue Desk agents after conversion, and the same client number retained through conversion when practical.
- [`src/retell/`](src/retell/README.md) is the public, sanitized historical workspace for the two managed Retell agents. It preserves separate public summaries, lifecycle observations, and offline validation without publishing runtime-derived control names, raw prompts, routing, runtime values, live identifiers, or deployable configuration.
...
- [`docs/zoho/standards/call-reporting-metric-contract.md`](docs/zoho/standards/call-reporting-metric-contract.md) defines call, qualification, transfer, reconciliation, estimated-value, and verified-value metrics plus one-client, freshness, privacy, and delivery gates.
...
- [`docs/runbooks/shared-seven-day-monitor-number-routing.md`](docs/runbooks/shared-seven-day-monitor-number-routing.md) defines the Development setup, number-to-deployment resolver, neutral fallback, post-call resolution, two-client isolation test, report partition, conversion, containment, and rollback sequence for the shared-monitor topology.
