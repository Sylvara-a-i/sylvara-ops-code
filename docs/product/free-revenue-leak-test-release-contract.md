...
The disabled field-setup candidate requires at most one additive, currently unprovisioned table: `RevenueLeakTestFieldSetupJourneys`. It uses a strict `recordType` discriminator for the exact canonical journey projection and narrow `current_control`, `control_operation`, `number_inventory`, `reservation_receipt`, `verification_attempt`, and `verification_window` coordinator families. The request-form store must strip the storage envelope and every coordinator field before its executable exact journey validator. One injected `stateCoordinator` must provide a serializable transaction domain within this table for control-fence compare-and-set, number claim, server-owned monotone window-attempt allocation, gateway consumption, and replay receipts. If the target store cannot prove that contract, installation remains blocked; passing source tests is not provisioning evidence.
...
This addition is source-only and currently `NOT_READY`. It keeps existing Form 1 and Form 2, creates no Form 3, adds no seventh Catalyst function, and treats the mobile web journey as a temporary operator-led session rather than a client portal. The CRM buttons now send protocol ID `free_revenue_leak_test_field_setup_v1` and numeric launch schema version `1`, matching the Catalyst contract. The journey uses a 256-bit single-use fragment nonce, digest-only storage, Gabriel's authenticated Development context, record-bound Lead/converted-Deal resume digests, exact client/server protocol ID and version headers with matching success-response echoes, and a bounded Secure/HttpOnly/SameSite session. Guarded transitions require exact server-authoritative receipts and compare-and-set readback; the browser cannot supply authority. Exact Form destinations are privately injected and constrained to their exact paths on the single public-host allowlist entry `forms.zohopublic.com`. The default request-form composition registers no routes and the default client remains a synthetic zero-network preview. An injected request composition claims exactly six dark source routes: launch, exchange, status, operator decision, conversion preview, and conversion confirmation. Conversion confirmation remains locked until the exact bounded preview is loaded for display. A lost browser mutation response permits one exact same-body retry. The server may replay only the action-bound immediately prior committed transition, including the original Form navigation receipt, without another compare-and-set; older, different-action, or conflicting evidence fails closed, and status-only readback cannot claim success. An immediately prior preview revision can only reread the already committed confirmation preview, not rebuild it. A legitimate authoritative pre-write change atomically refreshes only the current preview receipt at the same confirmation state with revision +1; the old receipt cannot write, while the refreshed display can be confirmed. Conversion confirmation uses only its dedicated exact-request replay path; `write_started`, an ambiguous result, and `reconciliation_required` remain operator-stop outcomes and are never masked by status.

The hosted client requires exactly ten distinct same-origin injected paths: five browser-consumed request-form paths (`exchange`, `status`, `operator decision`, `conversion preview`, and `conversion confirm`) plus the five setup-operation paths. The sixth request-form route, launch, is server-to-server and is not a client runtime path. Forwarding instructions use `POST` with the exact body `{journeyRevision, view}`, where `view` is `enable` or `rollback`.

The existing setup-form handler has a separate explicit-injection-only composition for exactly five dark source operations: existing-number status, existing-number claim, reviewed forwarding instructions, verification-window issue, and setup control. Its single authoritative `stateCoordinator` exposes `readNumberReservationStatus`, `readNumberReservationReceiptByOperationFingerprint`, `claimExistingAvailableNumberWithControlFenceAtomically`, `issueWindowWithControlFenceAtomically`, `readLatestWindowByOperationScopeFingerprint`, `applyControlIntentAtomically`, and `readControlOperationByOperationFingerprint`. The authenticated current-control aggregate carries nullable `latestControlOperationFingerprint`, so no-op and lost-response replay reads the exact immutable operation receipt instead of using a nonunique binding. Number-claim idempotency uses a stable operation fingerprint over the claim-route identity and exact client/environment/journey/deployment/configuration reservation binding, excluding session, revision, number, control scope, and fence. The claim transaction atomically updates current-control number, number state, aggregate binding, fence, and audit timestamp; its receipt records the accepted pre-claim fence and recomputed post-claim aggregate fence; exact replay accepts only those fences and never creates a second inventory transition or receipt. Number state and approved QA-caller fingerprint participate in the aggregate binding/fence, so either rotation invalidates older verification evidence. Number claim and window issue validate the current control fence atomically. Window issue uses a stable typed `operation_scope_fingerprint`; the coordinator alone allocates a monotone `attempt_epoch`, replays the exact unexpired latest window, or expires it and issues the next attempt. The browser setup-control actions are `confirm_forwarding_enabled`, `confirm_rollback_ready`, `stop`, and `resume`; instruction issuance is an internal server action on the same store. A committed stop changes the control fence but never claims that the customer's physical route was rolled back. Current control stores a raw 64-character fence, while window/call/receipt evidence stores `control_fence_<raw>`; the gateway atomic transaction requires exact typed-call = typed-window = prefix + raw-current-control equality. Missing, malformed, or stale current control returns `stale_control_fence` without consumption. Window and gateway authoritative-call/receipt contracts also bind provider and exact instruction evidence. The default composition claims no operation routes and preserves the six existing Form 2 routes. No provider client, number purchase, activation, live-route mutation, or setup-side verification-window consumption adapter is supplied. Neither composition can approve or activate a test.
...
`call_gap_monitor_v1` remains unchanged. The distinct `call_gap_capture_handoff_v2` source remains `NOT_READY`; its capability profile is Draft, disabled, unwired, bound to no traffic environment, and not importable from public source. Authenticated read-only schema inspection confirmed the transfer-node/update contract. An ignored private candidate is now an actual connected 15-node v2 graph with 15/15 reachability, no ordinary directed cycle, and exactly one bounded warm-transfer node reachable only after the policy gate and explicit caller acceptance; its failure branch uses exact `Transfer failed` handling. Routine, immediate-danger, and persistent-sensitive paths cannot reach transfer. The candidate contains exactly 17 bounded analysis fields and six lifecycle/post-call webhook events. Its build and inspection made zero network or provider interactions, and it remains private, disabled, unpublished, unbound, non-importable, and unauthorized for import. The source also defines an order-independent handoff state machine, authoritative intent/eligibility/destination/loop gates, required injected interfaces for a durable cumulative event ledger and one monotone Catalyst notification row, irreversible payload-null suppression for sensitive/nonactionable outcomes, 100 deterministic synthetic scenarios, mutation/network guards, and a strict minimizing event parser. No live ledger, notification, delivery, route, webhook ingress, or active provider adapter is supplied; exact sanitized live event fixtures, Draft import/readback, authentication/delivery integration, and live behavior remain unverified.
