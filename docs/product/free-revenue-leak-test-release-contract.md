...
The machine-readable Forms matrix is maintained in [`forms-manifest.json`](../../src/zoho-forms/free-revenue-leak-test/forms-manifest.json). It separates required source policy from sanitized read-only live observations; private field aliases, destinations, URLs, and identifiers are not publication evidence.

Form 1 uses `Intake_Submission_ID` as the canonical primary idempotency key and case-insensitive Email only as the ordered fallback after no primary match. Empty form values do not overwrite populated Lead data, and the existing CRM workflow remains the sole owner of the review task. Consent timestamps and versions, request timestamp, form version, offer, status, channel, source, and attribution remain hidden or locked audit data. The assisted controller intentionally omits the redundant `assisted_by` output; assisted provenance is represented by `Submission_Channel` and `Source_Page`. A connector-first 2026-08-28 preflight found no purpose-built Zoho Forms connector, so the authenticated read-only browser fallback inspected only the recorded builder fields without saving or submitting. Private readback independently matched the expected business account and Forms tenant namespace; no identity or namespace value is committed. The expected tenant showed `Intake_Submission_ID` as a non-respondent Zoho Forms-generated alphanumeric identifier with length 10 and repetition restriction enabled. Provider retry/re-push stability and duplicate-Lead behavior still require synthetic proof, and the exact field-by-field privacy dictionary remains unresolved. Form 1 has no configured field aliases, its assisted token is not mandatory, personal, or encrypted, and neither Form has the required Prefill-Webhook field configured. Sanitized evidence is recorded in [`read-only-browser-fallback-preflight-2026-08-28.json`](../../src/zoho-forms/free-revenue-leak-test/evidence/read-only-browser-fallback-preflight-2026-08-28.json).

Form 2's executable contract has exactly 33 client fields: 25 unconditionally required, three conditionally required with a null-when-inapplicable rule, and five optional. The exact `requestedStartDate` webhook key remains present, but null or blank normalizes to null and a supplied value must be a valid `YYYY-MM-DD` date. `alertRecipientEmail` is unconditionally required; `testPhoneNumber` and `alertRecipientMobile` are prohibited respondent and webhook inputs. The expected-tenant read-only fallback observed an active JSON webhook with all 36 required server-plus-client parameters plus both prohibited extras, and the form retained the corresponding test-number/mobile-SMS fields. It also observed `setupToken` as not mandatory, personal, or encrypted; `prefillId` as not mandatory; and both read-only flags as unresolved. Native Forms email/SMS notifications, approvals, and Zoho Sign remain excluded; Catalyst owns the one bounded email-proof workflow. Proof acceptance is controller-owned evidence: the one-time proof must be consumed, then the exact controller receipt and persisted CRM submission evidence must read back. There is no standalone CRM proof Boolean, and the proof is neither a signature nor go-live approval.
...
Terminal settlement creates an encrypted, sanitized `sync_report_summary` row in `CRMBillingOperations`. New rows use schema/domain `sylvara.crm-report-summary.v2`, which explicitly represents incomplete workflow-failure evidence as null. The consumer preserves v1 read compatibility only for legacy rows with a non-null workflow-failure count. Each versioned identity binds environment, Deal, deployment, configuration, report schema, canonical call-set digest, the full canonical report-revision digest, and action. A legitimate late event therefore creates a new immutable report revision; it cannot overwrite or collide with the completed prior row. Deploy and read back the v1/v2-capable consumer before the v2 producer; keep call ingress dark until isolated synthetic compatibility evidence passes.
...
The subscription proposal records `Plan`, `Billing_Frequency`, `Monthly_Recurring_Revenue`, `Setup_Fee`, `Subscription_Start_Date`, `Subscription_Acceptance_Status`, and `Subscription_Acceptance_Version`. `Subscription_Start_Date` is the operator-requested commercial date, not Billing-owned status. The authoritative Billing start date remains external provider readback and must exactly equal that requested CRM value. The immutable 2026-08-14 snapshot verifies the CRM field as a date but shows it absent from the then-active Standard layout. A fresh sanitized 2026-08-28 read-only preflight now proves the field is present, writable, API-create/update enabled, active-layout associated, and transition-available. That closes the current metadata/layout issue without making the proposal transition deployable: the inactive Blueprint topology, validators, save/readback contract, and synthetic acceptance remain blocked, and the full metadata/layout readback must be repeated immediately before deployment. No replacement field is inferred.

The CRM acceptance version must use the exact `terms-v1:<64 lowercase hex>` content address produced from the private commercial-terms currency, one-month interval and unit, common connected-minute rate, and all three monthly recurring/setup pairs. Closed Won remains a later manual transition only after a fresh, non-creating `reconcile` action recomputes the durable operation fingerprint from the current Plan immutable API value and canonical mapping, frequency, recurring amount, setup amount, requested start date, acceptance version, deployment, and configuration. The same evidence must read back the completed operation, exact Billing resources and commercial shape, and exact CRM integration state. `Subscription_Status` must be `Active`, `Billing_Automation_Status` must be `Paid Verified`, IDs and sync evidence must be populated, and `Billing_Automation_Error` must be present in readback and empty.

**Record Internal Approval** now references a machine-readable `internal-approval-receipt-v1` contract, and **Activate Test Route** references a separate `route-activation-readback-v1` contract. The first requires a freshly read-back, cryptographically authenticated approval intent and immutable receipt bound to the current Deal, deployment, configuration, route fingerprint, source revision, and deployment version while activation and timing remain absent. The second requires a fresh one-time receipt that chains the exact approval event/hash to the activation intent and receipt, binds the authoritative provider-route readback, proves both route registry and provider route active, and matches the CRM approval/start timestamps and exact seven-day interval. Only domain-separated keyed digests cross either evidence boundary; raw private identifiers, receipt values, provider values, and source revisions do not. HMAC intent authentication is an engineering control, not a legal or electronic signature. Both validators remain `not_implemented`, so the Blueprint is explicitly nondeployable.

Every Closed Lost path requires `Rollback_Completed_At` plus a fresh machine-readable external readback proving the route is inactive; an empty `Test_Start_At` is never sufficient. **Complete Free Test** reads back every terminal report field. Fields that the report contract legitimately permits to be null are required to be present and exactly null when applicable rather than being forced nonempty.

The 2026-08-24 sanitized metadata summary proves only its historical 17-field count. The later [field-by-field read-only preflight](../../src/zoho-crm/free-revenue-leak-test/evidence/live-topology-layout-preflight-2026-08-28.json) resolves exactly the 25-field gap derived from the immutable 2026-08-14 snapshots, including exact live types, JSON types, write flags, active-layout association, transition availability, and required immutable picklist API values. Self-authored release field lists still do not extend authoritative metadata, and a fresh readback remains mandatory immediately before deployment. Field readiness is currently closed; deployment remains blocked because the live candidate is inactive, its required `Revenue Desk Sales` pipeline binding is unproven, it lacks `Record Internal Approval` and `Activate Test Route`, it retains the combined legacy `Approve Go Live` transition, and it has no complete validator, typed save/readback, caller-inventory, or synthetic runtime proof.
...
Route control is an out-of-band private operator workflow, not a seventh function, fourth gateway route, or fifth worker mode. Approval records an immutable receipt bound to the exact active configuration, deterministic route fingerprint, source revision, and observed row version, then moves the deployment only to `Scheduled`. The CRM approval transition also requires fresh one-time keyed evidence of that exact signed intent and receipt; mutable CRM approval fields alone are insufficient. A separate activation decision is allowed only after fresh authoritative provider-route readback; its receipt chains to approval, and only then may the deployment become `Live` with `ACTUAL_START_AT` and `EXPIRES_AT` separated by exactly 604800000 milliseconds. The CRM activation transition separately requires a fresh one-time keyed proof of that approval-to-activation chain and active route readback; `Test_Start_At` or status strings alone are insufficient.
...
The Analytics release must produce exactly five sanitized record types: `deployment`, `call`, `daily_metric`, `final_test_result`, and `conversion_status`. `ENGAGEMENT_TYPE` is the originating engagement and immutable dashboard partition: a free-test conversion remains `free_test`, while the distinct `TARGET_ENGAGEMENT_TYPE=paid_service` field records the destination commercial state. It must create or uniquely verify the exact root folders **Revenue Desk - Data Model**, **Revenue Desk - Operations**, and **Revenue Desk - Customer Results**, independently read back the model contract's one-folder placement for all 31 canonical views, and create and independently read back the exact dashboard titles **Free-Test Operations Dashboard** and **Customer Results Dashboard** under the [dashboard contract](../../src/zoho-catalyst/revenue-desk-analytics/config/dashboard-contract.json). The existing workspace and assets are migration evidence and may be reused only after private identity, schema, permission, share, row-count, folder-membership, and watermark readback; they are not overwrite or deletion authority.
