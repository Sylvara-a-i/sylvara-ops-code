...
6. complete the [key-rotation contract](free-revenue-leak-test-key-rotation-contract.json) and [operator runbook](../runbooks/free-revenue-leak-test-key-rotation.md), revoke every exposed previous key or grant, prove old-key rejection with no overlap, and keep Production credentials independent.
