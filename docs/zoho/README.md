...
The consolidated Catalyst/Zoho release is **NOT READY FOR RETELL AGENT TESTING**. On 2026-08-27, the six canonical Development function definitions were deployed from revision `7fb101d60e4480a2aaa88de70d82d6b1ddc9e989` with no operator invocation performed and independently read back with exact source-revision stamps, Node 24, 256 MB, safe CRM/Analytics gates, authenticated POST-only direct-function Security Rule posture, API Gateway disabled, both 512 MB Function Job pools present, and the retry Cron absent. Connector-first discovery exposed no source/archive download operation, so authenticated first-party Catalyst UI Download supplied the six archive pullbacks; all six matched their exact uploads byte for byte by SHA-256 and length, while digests and private paths remain outside Git. CRM has 45 live variable names versus 42 public-registry names, and Analytics has 54 versus 26; the extra names remain privately unclassified. Analytics disabled mode is configuration readback only. CRM paid/probe gates do not disable `sync_report_summary`, so no-invocation evidence is not proof that the callable surface is inert. This proves exact-upload archive parity, but not configuration-registry parity, route-count or negative direct-caller inventory, twelve-route API Gateway parity, Job-target/Cron or legacy-caller binding, synthetic lifecycle acceptance, Retell behavior, cleanup, rotation, final-main, or dark-Production state. The prior four-table/two-function proof remains migration evidence only. The [final release contract](../product/free-revenue-leak-test-release-contract.md), [sanitized deployment evidence](../../src/zoho-catalyst/evidence/free-revenue-leak-test-development-six-function-deployment-2026-08-27.json), and [current reconciliation](../runbooks/free-revenue-leak-test-e2e-reconciliation-2026-08-24.md) are authoritative within those limits.
