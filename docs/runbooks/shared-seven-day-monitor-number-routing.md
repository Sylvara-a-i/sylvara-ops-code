...
Routine actionable calls remain email-only. Read-only public Retell schema and documentation confirm native transfer configuration families and canonical lifecycle event names, but eligible urgent, existing-customer, and specific-person handoff remains provider-neutral until exact Draft mapping, complete payload variations, and the provider parser are reviewed. The candidate requires injected durable cumulative event-ledger and monotone notification-store interfaces but supplies no live adapter or delivery path. Sensitive/nonactionable outcomes reconcile to an irreversible payload-null suppression tombstone so delayed or concurrent actionable input cannot retain or resurrect notification content. The setup controller may issue one exact 300-second route-verification window after atomically expiring any stale `Open` row; only the call gateway may atomically consume or expire it. Route verification must still be intercepted before normal agent intake and create no handled-call count or notification. Until exact provider parsing and live route behavior are proven, v2 and route verification remain `NOT_READY`.
