...
Routine actionable calls remain email-only. Authenticated read-only Retell schema inspection confirms the transfer-node/update contract, and the v2 source now includes a strict, unwired parser for the four documented lifecycle event names with a private keyed fingerprint boundary. The source remains `NOT_READY`; eligible urgent, existing-customer, and specific-person handoff remains inactive until an isolated Draft is imported/read back and exact sanitized payload variations are accepted. The candidate requires injected durable cumulative event-ledger and monotone notification-store interfaces but supplies no live adapter or delivery path. Sensitive/nonactionable outcomes reconcile to an irreversible payload-null suppression tombstone so delayed or concurrent actionable input cannot retain or resurrect notification content. The setup controller may issue one exact 300-second route-verification window after atomically expiring any stale `Open` row; only the call gateway may atomically consume or expire it. Route verification must still be intercepted before normal agent intake and create no handled-call count or notification. Runtime v2 and route verification remain `NOT_READY`, disabled, unwired, and unbound.
