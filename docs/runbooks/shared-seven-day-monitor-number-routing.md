...
The source-only `call_gap_capture_handoff_v2` candidate is governed by [ADR 0008](../adr/0008-bounded-free-test-human-handoff-and-operator-led-field-setup.md) and the [field-setup runbook](free-revenue-leak-test-field-setup.md). It does not alter `call_gap_monitor_v1`, reserve or purchase a number, bind traffic, import a Retell Draft, or authorize a call. The existing setup-form handler can claim five dark source-only setup-operation routes only through an explicit injected composition; its default claims none. Forwarding instructions are `POST {journeyRevision, view}` with `view` restricted to `enable` or `rollback`. One authoritative `stateCoordinator` owns control-fenced existing-number claim, immutable reservation-receipt readback, setup-control replay, and verification-window issue; current control carries a nullable latest-operation pointer so control replay reads the exact immutable receipt by operation key rather than binding. The number-claim operation key is stable across session/revision/fence rotation; the claim transaction also updates the current-control number, number state, binding, fence, and audit timestamp, while its receipt binds both accepted pre-claim and recomputed post-claim fences to prevent a second transition on replay. Number state and approved QA caller are safety inputs to the aggregate fence, so their rotation invalidates older windows. It is source-only and has no live store mapping. The setup composition has no provider client, purchase, activation, live-route mutation, or verification-window consumption adapter.

Routine actionable calls remain email-only. Authenticated read-only Retell schema inspection confirms the transfer-node/update contract, and an ignored private candidate now realizes the v2 design as a connected 15-node graph with 15/15 reachability, no ordinary directed cycle, and exactly one bounded warm-transfer node reachable only through the policy gate and explicit caller acceptance. Its failure branch uses the exact `Transfer failed` edge; routine, immediate-danger, and persistent-sensitive paths cannot reach transfer. The candidate has exactly 17 bounded analysis fields and six lifecycle/post-call events, was built and inspected with zero network or provider interactions, and remains private, disabled, unpublished, unbound, and non-importable. The v2 source also includes a strict, unwired parser for the four documented transfer lifecycle event names with a private keyed fingerprint boundary. The source remains `NOT_READY`; eligible urgent, existing-customer, and specific-person handoff remains inactive until an isolated Draft is imported/read back and exact sanitized payload variations are accepted. The candidate requires injected durable cumulative event-ledger and monotone notification-store interfaces but supplies no live adapter or delivery path. Sensitive/nonactionable outcomes reconcile to an irreversible payload-null suppression tombstone so delayed or concurrent actionable input cannot retain or resurrect notification content. The setup controller may issue one exact 300-second route-verification window through `issueWindowWithControlFenceAtomically` after atomically expiring any stale `Open` row. A stable server operation scope may allocate only a coordinator-owned monotonically increasing attempt epoch; the browser cannot choose either value. Window, authoritative-call, and receipt evidence binds the current control fence, provider, and exact instruction evidence. Only the call gateway may call `consumeOpenWindowAtCurrentControlFence`; `stale_control_fence` cannot consume a window. A setup stop changes the fence but never claims physical forwarding rollback. Route verification must still be intercepted before normal agent intake and create no handled-call count or notification. Runtime v2 and route verification remain `NOT_READY`, disabled, unpublished, unwired, unbound, non-importable, and unauthorized for runtime use.
