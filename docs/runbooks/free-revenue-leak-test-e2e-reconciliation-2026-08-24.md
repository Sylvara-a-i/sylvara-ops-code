...
| Call worker | One private Job target with four explicit modes | The Development Job definition has exact aab7c18 source/archive parity, and first-party UI readback shows exactly zero variables. Static configuration fails closed; the execution did not invoke a Job to prove runtime behavior. Canonical Job-target binding, legacy caller retirement, and all four live mode contracts remain unproven. | P1 blocked |
