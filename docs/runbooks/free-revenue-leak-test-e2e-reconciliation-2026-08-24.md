...
| CRM/Billing | Accepted-only TEST subscription path with authoritative readback | The Development definition and its 30-variable approved private map have exact aab7c18 source/readback parity. Paid preparation and compatibility probing remain false, and the rotated artifact-bound runtime proof read back exactly. No CRM or Billing write or compatibility invocation occurred. | P1 blocked |
| Analytics | Canonical `analytics_sync`, checkpoints, outbox, dashboards, retry, and freshness | The Development Job definition and its minimized 7-variable approved private map have exact aab7c18 source/readback parity with disabled mode. The operator invoked no Job or runtime `DisabledNoOp`. Canonical Job binding, provider migration, freshness, and dashboard acceptance remain unproven. | P1 blocked |
