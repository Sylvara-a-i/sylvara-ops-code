...
| --- | --- | --- | --- |
| Form 1 | Canonical function and one canonical session table | Source and local tests exist; target-project function, routes, variables, hosted Form, and CRM caller cutover are not read back | P1 blocked |
| Form 2 | Canonical function and four canonical v3 stores | Source and local/hosted tests exist; target function and hosted callers are not cut over; v2 and probe artifacts remain | P1 blocked |
...
| Canonical call stores | Five `RevenueDesk*` tables with immutable version authority | The sanitized Development readback found 29 tables, including nonzero predecessor call/configuration tables; no classification, key mapping, digest, or canonical migration proof exists | P1 blocked |
...
| CRM/Billing | Accepted-only TEST subscription path with authoritative readback | Read-only live CRM metadata confirms all 14 previously proposed Deal fields already exist. Billing TEST has the Revenue Desk product and free-evaluation plan; the live catalog has the three paid plan families and active usage item. Exact safe catalog binding and the disposable TEST customer/subscription positive path remain unproven. | P1 blocked |
| Analytics | Canonical `analytics_sync`, checkpoints, outbox, dashboards, retry, and freshness | Development currently has 10 `AnalyticsSyncCheckpoints` rows and 307 `AnalyticsSyncOutbox` rows; ownership, payload generation, active readers/writers, Job pool, live connection, destinations, and dashboard readback are not proven | P1 blocked |
...
### Sanitized Development Data Store readback

The corrected paginated Development readback on 2026-08-24 used the server-supported 300-row page size, continued past the first page, and observed 29 tables. Sixteen were nonzero:
...
| Creator target | Creator Custom API inventory remains unverified because the available connector and UI access could not authenticate. | Live deployment and duplicate removal remain blocked until exact Custom API ownership, auth, and rollback are read back. |
...
5. Prove CRM workflow and Blueprint through a fresh synthetic Lead-to-Deal lifecycle.
