...
| --- | --- | --- | --- |
| Form 1 | Canonical function and one canonical session table | Canonical source and an empty session table exist. The hosted title, consent/submission timestamps, and fixed audit versions are configured; native CRM upsert remains the sole writer. Consent/confirmation copy, assisted prefill, function/routes/variables, and synthetic CRM readback remain blocked. | P1 blocked |
