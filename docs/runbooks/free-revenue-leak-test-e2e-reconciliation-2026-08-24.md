...
| Call worker | One private Job target with four explicit modes | The Development Job definition has exact aab7c18 source/archive parity. A single-use attempt to install the approved 28-variable map was consumed without a deterministic operator-visible orchestration result or private in-packet execution-status record; no retry occurred. Independent readback showed the Job still exact at Node 24, 256 MB, and zero variables, so the map did not persist and no partial map remained. Static configuration fails closed; the operator did not invoke a Job to prove runtime behavior. Provider-complete post-attempt Job and direct-caller inventory remains unproven, so a transient active binding or invocation cannot be excluded. Canonical Job-target binding, legacy caller retirement, and all four live mode contracts remain unproven. | P1 blocked |
