...
The six canonical function definitions are now present in Catalyst Development at revision `7fb101d60e4480a2aaa88de70d82d6b1ddc9e989`, with exact source-stamp, Node 24, 256 MB, safe-gate, Security Rule, pool, scheduler, and bounded provider-log readback. All six Catalyst-pulled archives match their exact uploaded archives byte for byte by SHA-256 and length; digests and private paths remain outside Git. The complete function inventory contains exactly six canonical and six known legacy definitions. The provider's generic `is_deployed` flag is false for every canonical definition and is recorded as a provider/environment-state field, not treated as proof that Development source is absent or that Production was deployed. Four pools exist: the two exact canonical Function/512 pools and two noncanonical pools. One inactive Cron targets a legacy function through a noncanonical pool; no active Cron or canonical-pool Cron reference exists, and `RevenueDeskRetry1m` remains absent. No operator function or Job invocation was performed. Provider logs contain zero access and application records in the 24-hour post-update window covering all six latest definition updates, but the seven-day retention window contains 24 older access records on only the two preexisting definitions. That proves bounded zero post-update log activity, not a complete caller inventory or callable-surface inertness. CRM has 45 live variable names versus 42 public-registry names, and Analytics has 54 versus 26; extra names remain privately unclassified and omitted. Analytics disabled mode is configuration readback only, not a runtime `DisabledNoOp` proof. CRM paid/probe gates do not disable `sync_report_summary`. Read-only Retell reconciliation proves the provider-neutral variable, post-call analysis, webhook-event, and timeout contracts, but the phone webhook remains on the legacy Catalyst boundary and the required no-retained-content, pre-assent media/DTMF, and static-notice controls remain unproven. No Retell agent change, test, simulation, call, publish, or route change occurred. Final-main/configuration parity, route count, twelve-route API Gateway parity, canonical Job binding, legacy-caller retirement, synthetic runtime acceptance, Retell acceptance, migration/cleanup, rollback, and dark-Production proof remain release blockers.
...
| --- | --- | --- | --- |
| Form 1 | Canonical function and one canonical session table | Canonical source and an empty session table exist. The Development definition has exact source-stamp/Node 24/256 MB readback with zero variables and a POST-only authenticated direct-function Security Rule, but API Gateway routes, assisted prefill, variables, caller cutover, invocation, and synthetic CRM readback remain blocked. The hosted title, consent/submission timestamps, fixed audit versions, exact calls-and-email consent copy, and truthful confirmation copy are configured and independently read back; native CRM upsert remains the sole writer. | P1 blocked |
| Form 2 | Canonical function and four canonical v3 stores | Canonical source and the hosted title/scope/confirmation copy exist. The Development definition has exact source-stamp/Node 24/256 MB readback with zero variables and a POST-only authenticated direct-function Security Rule, but that containment does not provide the six-route contract or required GET access route. The legacy controller remains the active webhook rollback path; required fields, variables, callers, and cutover remain blocked. | P1 blocked |
| Call gateway | Fast verification, durable receipt, Job submission, and readiness only | The canonical Development definition has exact source-stamp/Node 24/256 MB readback with zero variables and a POST-only authenticated direct-function Security Rule. API Gateway is disabled; none of the three manifest routes was exercised, and route parity and runtime acceptance remain unproven. | P1 blocked |
| Call worker | One private Job target with four explicit modes | The canonical Job definition has exact source-stamp/Node 24/256 MB readback with zero variables. Both canonical Function/512 pools exist and have no Cron reference. The only observed Cron is inactive and targets a legacy function through a noncanonical pool. The operator performed no Job invocation; canonical Job-target binding, legacy caller retirement, and all four live mode contracts remain unproven. | P1 blocked |
...
| CRM/Billing | Accepted-only TEST subscription path with authoritative readback | The Development definition has exact source-stamp/Node 24/256 MB readback with 45 live variable names versus 42 public-registry names, paid preparation false, and the compatibility probe false. Extra names remain privately unclassified. The operator did not invoke the probe because no verified private Advanced I/O invocation channel was available. CRM has all 17 required Deal fields, but the workflow/Blueprint and Billing TEST catalog blockers below remain; no CRM or Billing lifecycle acceptance was added by this deployment. The report-summary action is not proven inert because negative caller inventory is incomplete. | P1 blocked |
| Analytics | Canonical `analytics_sync`, checkpoints, outbox, dashboards, retry, and freshness | The Development Job definition has exact source-stamp/Node 24/256 MB readback with 54 live variable names versus 26 public-registry names. Its mode read back disabled as configuration; no runtime `DisabledNoOp` was proven. Both canonical Function/512 pools exist without Cron references; the only Cron is inactive and legacy-targeted, and `RevenueDeskRetry1m` is absent. The operator performed no Job invocation; canonical Job binding remains unproven. Two plausible owned workspaces still make target binding ambiguous, neither dashboard exists, and version-2 provider write/readback, migration, freshness, and dashboard readback remain unproven. | P1 blocked |
| Development deployment | Exact final-main six-function artifact/runtime parity and synthetic E2E | All six canonical definitions are present exactly once alongside six known legacy definitions, with exact reviewed-revision source-stamp/runtime metadata readback and 6/6 archive-pullback parity to their uploads. The generic provider `is_deployed` flag is false on all six canonical definitions and is not interpreted as a source-installation or Production conclusion. Provider logs show zero access/application records in the 24-hour post-update window, but 24 older access records on the two preexisting definitions preserve the caller-inventory and inertness blockers. Configuration-registry parity, route-count readback, twelve-route API Gateway parity, canonical Job binding, legacy-caller retirement, invocation acceptance, synthetic E2E, final-main, and rollback acceptance remain incomplete. | P1 blocked |
...
1. Complete acceptance for the six exact-upload deployed definitions: classify the live-versus-registry gaps, prove configuration-registry parity, route count, exact API Gateway routes, Job-target/Cron and legacy-caller bindings, the disabled-first retry schedule, and direct invocation of the three call-gateway routes and four call-worker modes without Retell agent testing.
...
Current status is **NOT READY FOR RETELL AGENT TESTING**. The canonical Catalyst Development definitions are installed with exact-upload archive parity and bounded zero post-update provider-log activity, and read-only Retell reconciliation proves the provider-neutral contract. Historical access records, the legacy phone-webhook binding, unproven Retell storage/notice/media/DTMF controls, callable-surface inertness, final-main/configuration parity, routes, canonical Job binding, invocation acceptance, Forms/CRM/Billing/Analytics acceptance, migration, cleanup, Production, security, isolation, and rollback work remain.
