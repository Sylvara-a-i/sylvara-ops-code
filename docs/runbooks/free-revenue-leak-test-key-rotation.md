...
- a paid or report-summary CRM/Billing operation is processing or unresolved; a retained writer guard is busy, malformed, ambiguous, or lacks exact idle-state readback; a synthetic subscription/customer remains; or an old scope may be invoked again;
...
A retained writer guard intentionally keeps `STATUS=processing` while idle. A nonzero guard is migration-eligible only after independent readback proves `LAST_OUTCOME=report_writer_idle`, `inflightOperationKey=null`, the exact `confirmedOperationKey` reference state, raw CRM binding, version, status, count, and environment. A busy, malformed, or ambiguous guard blocks rotation.
...
5. Rotate durable derivation keys. Recompute every retained number lookup from its authoritative private number, then regenerate and read back route approval and activation evidence. Never partially rekey a call graph, Analytics partition, the retained report-summary namespaces v1 (`sylvara.crm-report-summary.v1`), v2 (`sylvara.crm-report-summary.v2`), and current v3 (`sylvara.crm-report-summary.v3`), or the separate writer-guard v1 namespace (`sylvara.crm-report-writer.v1`). Retained summary rows require exact old-to-new `OPERATION_KEY`, `OPERATION_FINGERPRINT`, private `OPERATION_PAYLOAD_JSON`, call-set digest, full report-revision digest, v3 `sourceVersions` where applicable, CRM summary, and deployment-marker reconciliation. Retained writer-guard rows independently require exact `OPERATION_KEY`, `OPERATION_FINGERPRINT`, private `OPERATION_PAYLOAD_JSON`, CRM binding, `confirmedOperationKey`, `inflightOperationKey`, version, status, and count. Terminal report-summary acceptance does not establish writer-guard acceptance.
...
- `ANALYTICS_PARTITION_HMAC_SECRET` changes client/deployment partitions, the CRM conversion record key under `revenue-desk-analytics-conversion-v2`, every retained report-summary v1/v2/current v3 operation key, fingerprint, call-set digest, and full report-revision digest, plus every separate writer-guard v1 HMAC key/fingerprint and confirmed/in-flight summary-operation reference stored in `CRMBillingOperations`. A partition-only replacement preserves each v3 `sourceVersions` vector and its underlying `CALL_KEY` / `CALL_VERSION` inputs, and preserves each writer guard's raw CRM binding fields; it rekeys the enclosing HMAC report-summary and writer-guard identities and pointers. The conversion fact keeps `free_test` as its origin partition and `paid_service` as its separate target; rotation must not rewrite that classification. Summary-terminal checks and writer-guard acceptance are separate gates. Both call-runtime and CRM/Billing producers, the report dispatcher/consumer, and the Analytics consumer must never run with mixed versions.
- Generate the replacement `ANALYTICS_PARTITION_HMAC_SECRET` only within a separately approved owner handoff. Its shared executable contract is 32-256 non-whitespace characters and 32-256 UTF-8 bytes; use a fresh owner-generated 64-character ASCII value. This recommendation does not authorize rotation, migration acceptance, deployment, or activation, and an exposed previous value must never be reused or recorded.
