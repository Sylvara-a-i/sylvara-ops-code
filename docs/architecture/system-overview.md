...
The shared `7-Day Free Test` agent remains the product identity; dedicated number plus exact client/deployment/configuration ownership remains the tenant boundary. `call_gap_monitor_v1` is unchanged. `call_gap_capture_handoff_v2` is Draft, disabled, bound to no traffic, and provider-neutral only. Human handoff, Retell infrastructure fallback, and customer phone-system rollback are separate configuration concepts. The public Retell configuration families and canonical transfer lifecycle event names are confirmed. Source now separates server-issued verification windows from gateway-only atomic consumption/expiry and requires injected durable event-ledger plus monotone notification-store interfaces, including irreversible payload-null suppression tombstones; no live mappings are supplied. Exact provider parsing, authenticated Draft mapping, and pre-agent route behavior remain `NOT_READY`; live installation is `NOT_AUTHORIZED`.
