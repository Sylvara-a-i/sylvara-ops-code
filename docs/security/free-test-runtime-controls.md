...
The v2 handoff contract persists only bounded state and destination/recipient fingerprints. Structured provider events outrank post-call analysis; a model cannot claim bridge or success. Routine calls do not transfer. Configuration failures collect no caller data. Immediate-danger, sensitive-data, spam, vendor, applicant, invalid-destination, and route-loop cases fail closed. One durable Catalyst notification is allowed only for an actionable call; Retell sends none. Exact provider parsers and live behavior remain `NOT_READY` and uninstalled.
