...
## Field-Setup And Handoff V2 Source Controls

The [field-setup runbook](../runbooks/free-revenue-leak-test-field-setup.md) adds no active route. Its launch candidate uses protocol ID `free_revenue_leak_test_field_setup_v1`, numeric schema version `1`, 256 bits of entropy, keyed-digest-only nonce/session storage, a 60-second maximum launch lifetime, exact Development/operator/record binding, single-use exchange, and a bounded Secure/HttpOnly/SameSite cookie. A new journey begins only from a Lead; later Lead or converted-Deal launch resumes require the stored keyed record-bound digest and rotate launch/session material without creating a second journey. Browser exchange, status, decision, conversion-preview, conversion-confirm, and setup-operation requests must identify the exact canonical protocol ID and version, and successful responses echo both values. URLs contain only the fragment nonce. Form navigation is limited to exact privately injected paths on the sole allowlisted public host `forms.zohopublic.com`, with bounded opaque query values and prohibited redirect/record/contact keys. The web client removes the fragment before asynchronous work and cannot convert, reserve, verify, approve, activate, or mutate a live route without a separately authenticated server-side decision. The injected request composition has exactly six dark source routes; the client accepts exactly ten distinct same-origin injected runtime paths, and conversion confirm stays locked until its exact preview is loaded for display. The default listener claims no field-setup route, and the default client remains a synthetic zero-network preview.

The setup-form source can compose five operations only through explicit injected adapters: existing-number status, control-fenced atomic claim plus immutable receipt readback, reviewed forwarding instructions, control-fenced 300-second verification-window issue plus readback, and setup-control compare-and-set plus readback. Forwarding instructions require `POST {journeyRevision, view}` with `view` limited to `enable` or `rollback`. Browser control is limited to `confirm_forwarding_enabled`, `confirm_rollback_ready`, `stop`, and `resume`; instruction issuance remains server-internal. One authoritative `stateCoordinator` owns reservation, control, and window compare-and-set boundaries plus idempotency receipts. Current control stores nullable `latestControlOperationFingerprint`, and control receipt readback uses that exact operation key rather than the nonunique binding. A number-claim operation fingerprint excludes volatile session, revision, number, control-scope, and fence values; the same transaction updates the current-control number, number state, binding, fence, and audit timestamp, the receipt stores accepted pre-claim and recomputed post-claim fences, and exact replay cannot repeat the inventory transition. Number state and approved QA-caller fingerprint participate in the aggregate binding/fence, so either change fences an older verification window. The single permitted `RevenueLeakTestFieldSetupJourneys` table uses strict journey/control/operation/inventory/receipt/attempt/window record families; it is unprovisioned, and a future adapter must prove serializable cross-record control-fence validation across every mutation or remain dark. Current control stores the raw 64-character fence digest; windows, calls, and receipts store `control_fence_<digest>`. Gateway consumption strips the trusted prefix for the atomic current-control comparison and requires exact typed call/window equality before any consume or receipt. Missing, malformed, or mismatched current control returns `stale_control_fence` with no mutation. Number claim, window issue, and gateway consumption bind that fence; windows, authoritative calls, and receipts additionally bind provider and exact instruction-evidence fingerprints. A stable server-owned window operation scope may advance only through a coordinator-owned monotone attempt epoch. Stop changes setup state and the fence but never claims physical route rollback. The default composition claims none of those paths. No provider client, number-purchase boundary, activation or live-route mutation operation, or setup-side verification-window consumer exists in this composition.

The v2 handoff contract persists only bounded state and destination/recipient fingerprints. Structured provider events outrank post-call analysis; a model cannot claim bridge or success. Lifecycle convergence is order-independent and monotone, caller-supplied prior state is forbidden, and handoff requires authoritative intent, service/area eligibility, destination, fingerprint, loop evidence, and explicit caller acceptance. The durable ledger binding fixes the server-authoritative configured destination before consent and rejects a later target rebind without mutation. Routine calls do not transfer. Configuration failures collect no caller data. Immediate-danger, sensitive-data, spam, vendor, applicant, invalid-destination, and route-loop cases fail closed. The ignored private candidate realizes this boundary as a connected 15-node graph with 15/15 reachability, no ordinary cycle, one consent-gated transfer with exact `Transfer failed` handling, 17 analysis fields, and six events. Its build and inspection made zero network or provider interactions; it remains private, disabled, unpublished, unbound, and non-importable. The service requires an injected durable cumulative event ledger and an injected store that atomically reconciles one notification-disposition row before any delivery claim. Sensitive/nonactionable suppression outranks an actionable intent, uses a payload-null tombstone, and cannot be reversed by delayed or concurrent input; Retell sends none. The strict source parser verifies provider/call/agent/scope/target/configuration binding, requires a private keyed fingerprint boundary, bounds the full provider object, and emits only minimized normalized evidence. The committed source remains `NOT_READY` and supplies no live store, delivery, route, webhook ingress, or active provider adapter; exact sanitized webhook fixtures and provider readback are still missing, and runtime v2 remains unwired, unbound, and uninstalled.
